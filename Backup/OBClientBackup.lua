...
-- Forward decls for functions defined later
local arrowAndGateToCurrentStep, retargetCurrentStep
...
local BE_GridClick        = BE:FindFirstChild("OnboardingGridClick")
if not BE_GridClick then
	local ev = Instance.new("BindableEvent")
	ev.Name = "OnboardingGridClick"
	ev.Parent = BE
	BE_GridClick = ev
end
local BE_GridSelect       = BE:FindFirstChild("OnboardingGridSelect")
if not BE_GridSelect then
	local ev = Instance.new("BindableEvent")
	ev.Name = "OnboardingGridSelect"
	ev.Parent = BE
	BE_GridSelect = ev
end
local BE_GridSelectAccepted = BE:FindFirstChild("OnboardingGridSelectAccepted")
if not BE_GridSelectAccepted then
	local ev = Instance.new("BindableEvent")
	ev.Name = "OnboardingGridSelectAccepted"
	ev.Parent = BE
	BE_GridSelectAccepted = ev
end
...
-- Forward decl for mobile place-button helpers
local stopPlaceButtonPulse, updatePlaceButtonPulse, armPlaceButtonPulse, disarmPlaceButtonPulse
...
----------------------------
-- Mobile Place button pulse
----------------------------
local placeButtonWatcher = { btn = nil, conn = nil }
local placePulseArmed = false
local placePending = false
local function _isTouchOnly()
...
local function clearAllUIPulses()
	...
	arrowOwner = nil
	disarmPlaceButtonPulse()
	placePending = false
end

stopPlaceButtonPulse = function()
	clearPulseAndArrow("BM_PlaceButton")
end

updatePlaceButtonPulse = function()
	-- Only for true-touch devices; bail on PC
	if not _isTouchOnly() then
		stopPlaceButtonPulse()
		return
	end

	if not placePulseArmed then
		stopPlaceButtonPulse()
		return
	end
	...
	-- Resolve and cache the button from UITargetRegistry
	if not (placeButtonWatcher.btn and placeButtonWatcher.btn.Parent) then
		local btn = UITargetRegistry.Get("BM_PlaceButton")
		if btn and btn:IsA("GuiObject") then
			placeButtonWatcher.btn = btn
		end
	end

	local btn = placeButtonWatcher.btn
	...
	if btn.Visible then
		requestPulseAndArrow("BM_PlaceButton", UDim2.new(0,0,-0.12,0))
	else
		stopPlaceButtonPulse()
	end
end

armPlaceButtonPulse = function()
	placePulseArmed = true
	updatePlaceButtonPulse()
end

disarmPlaceButtonPulse = function()
	placePulseArmed = false
	stopPlaceButtonPulse()
end

local function ensurePlaceButtonWatcher()
	if placeButtonWatcher.conn then return end
	local btn = placeButtonWatcher.btn or UITargetRegistry.Get("BM_PlaceButton")
	if not (btn and btn:IsA("GuiObject")) then return end
	placeButtonWatcher.btn = btn
	placeButtonWatcher.conn = btn:GetPropertyChangedSignal("Visible"):Connect(updatePlaceButtonPulse)
	updatePlaceButtonPulse()
end

-- Arm place button pulse after each grid click (PC ignored via _isTouchOnly)
if BE_GridClick and BE_GridClick:IsA("BindableEvent") then
	BE_GridClick.Event:Connect(function()
		if not _isTouchOnly() then return end
		placePending = true
		armPlaceButtonPulse()
		retargetCurrentStep(true, true)
	end)
end

-- Mobile place confirmation clears the pulse until next grid click
if BE_MobileClick and BE_MobileClick:IsA("BindableEvent") then
...
-- Touch selection of a grid cell arms the place button; do not advance until placed
if BE_GridSelectAccepted and BE_GridSelectAccepted:IsA("BindableEvent") then
	BE_GridSelectAccepted.Event:Connect(function()
		if not _isTouchOnly() then return end
		placePending = true
		armPlaceButtonPulse()
		retargetCurrentStep(true, true)
...
-- Current target resolver (single source of truth for pulsing/gating order)
local function resolveCurrentTarget()
	...
	return guardItem or flowItem or currentGateId, flowState
end
...
arrowAndGateToCurrentStep = function(skipRoute)
	...
	-- Only gate on mobile: highlight Place when waiting for tap; PC skips this path.
	if _isTouchOnly() then
		if not placePending then
			disarmPlaceButtonPulse()
		end
		...
		if placePending then
			armPlaceButtonPulse()
			requestPulseAndArrow("BM_PlaceButton", UDim2.new(0,0,-0.12,0))
			clearBuildMenuPulsesExcept("BM_PlaceButton")
			return
		end
	end
	...
	local wantMajor = majorForItem(id)
	local wantHub   = hubForItem(id)
	-- Prefer the live BuildMenu state (it resets to Transport on reopen), then fall back to any committed click.
	local actualMajor         = getCurrentMajor(true)
	local committedMajorState = getCommittedMajor()
	local curMajor            = actualMajor or committedMajorState
	local committedHubState   = getCommittedHub(committedMajorState or actualMajor)
	local actualHub           = getCurrentHub(curMajor, true)
	local curHub              = actualHub or committedHubState

	-- On the B2 pipe step, force reroute only if not already on Supply/Water.
	if activeSeq == "barrage2" and id == "WaterPipe" then
		if curMajor ~= wantMajor then curMajor = nil end
		if curHub   ~= wantHub   then curHub   = nil end
	end
	dbg_route("routing id=%s wantMajor=%s wantHub=%s curMajor=%s curHub=%s",
		tostring(id), tostring(wantMajor), tostring(wantHub), tostring(curMajor), tostring(curHub))

	-- If user already has the correct tool selected AND is already on the right major/hub, bail early.
	-- If the menu reset to Transport (or wrong hub), we still need to guide them back through the path.
	if _G.OB_CurrentTool == id then
		local contextAligned = (not wantMajor or curMajor == wantMajor) and (not wantHub or curHub == wantHub)
...
-- Retarget UI to the current step (single entry point for pulses/arrows)
retargetCurrentStep = function(skipRoute, allowBuildNudge)
	clearBuildMenuPulsesExcept(nil)
	clearPulseAndArrow("BuildButton")

	local id = resolveCurrentTarget()
	if id then
		arrowAndGateToCurrentStep(skipRoute)
	elseif allowBuildNudge then
		requestPulseAndArrow("BuildButton", UDim2.new(0,0,-0.12,0))
	end
end
...
local function applyToggle(enabled)
	...
	if not _G.OB_ENABLED then
		...
		retargetCurrentStep(false, true)
...
DisplayGrid.OnClientEvent:Connect(function(mode)
	...
	-- Keep the current pulse/gate alive for barrage2/3 so the selected tool keeps pulsing.
	local inBarrage23 = (activeSeq == "barrage2" or activeSeq == "barrage3")
	if guardPulseItem and item == guardPulseItem and inBarrage23 then
		-- keep the guard pulse active
	else
		if guardPulseItem and item == guardPulseItem then Stop(guardPulseItem); guardPulseItem = nil end
	end
	if serverPulseItem and item == serverPulseItem and not inBarrage23 then
		Stop(serverPulseItem); serverPulseItem = nil
	end
	-- Only nuke all pulses when not in barrage2/3; otherwise keep guiding on the same tool.
	if not inBarrage23 then
		Stop("*")
		clearBuildMenuPulsesExcept(nil)
	end
...
StateChanged.OnClientEvent:Connect(function(stepName, payload)
	...
	if stepName == "Onboarding_StartBarrage1" then
		...
		if resumeAt <= 1 then
			retargetCurrentStep(false, true)  -- allow showcase route
		else
			retargetCurrentStep(true, true)   -- suppress showcase on resume
		end
		return
	end

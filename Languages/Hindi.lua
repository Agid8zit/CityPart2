return {
	-- Set default dialect to Hindi
	__default_dialect = "Khariboli",

	-- Numbers & Stuff: these remain untranslated and keep the placeholder

	["1"] = {
		["_default"] = "...",
		["Khariboli"] = "1",
	},

	["2"] = {
		["_default"] = "...",
		["Khariboli"] = "2",
	},

	["3"] = {
		["_default"] = "...",
		["Khariboli"] = "3",
	},

	["4"] = {
		["_default"] = "...",
		["Khariboli"] = "4",
	},

	["5"] = {
		["_default"] = "...",
		["Khariboli"] = "5",
	},

	["6"] = {
		["_default"] = "...",
		["Khariboli"] = "6",
	},

	["7"] = {
		["_default"] = "...",
		["Khariboli"] = "7",
	},

	["8"] = {
		["_default"] = "...",
		["Khariboli"] = "8",
	},

	["9"] = {
		["_default"] = "...",
		["Khariboli"] = "9",
	},

	["0"] = {
		["_default"] = "...",
		["Khariboli"] = "0",
	},

	["⬆️ UPGRADE"] = {
		["_default"] = "⬆️ UPGRADE",
		["Khariboli"] = "⬆️ UPGRADE",
	},

	["Boombox Song"] = {
		["_default"] = "Boombox Song",
		["Khariboli"] = "Boombox Song",
	},

	["CREDITS"] = {
		["_default"] = "CREDITS",
		["Khariboli"] = "CREDITS",
	},

	["Delete"] = {
		["_default"] = "Delete",
		["Khariboli"] = "Delete",
	},

	["No"] = {
		["_default"] = "No",
		["Khariboli"] = "No",
	},

	["Delete Save"] = {
		["_default"] = "Delete Save",
		["Khariboli"] = "Delete Save",
	},

	["Cancel"] = {
		["_default"] = "Cancel",
		["Khariboli"] = "Cancel",
	},

	["Yes"] = {
		["_default"] = "Yes",
		["Khariboli"] = "Yes",
	},

	["Offsale"] = {
		["_default"] = "Offsale",
		["Khariboli"] = "Offsale",
	},

	["To Purchase"] = {
		["_default"] = "To Purchase",
		["Khariboli"] = "To Purchase",
	},

	["x2 Earnings!"] = {
		["_default"] = "x2 Earnings!",
		["Khariboli"] = "x2 Earnings!",
	},

	["Collect"] = {
		["_default"] = "Collect",
		["Khariboli"] = "Collect",
	},

	["ON"] = {
		["_default"] = "ON",
		["Khariboli"] = "ON",
	},

	["Template"] = {
		["_default"] = "Template",
		["Khariboli"] = "Template",
	},

	["Settings"] = {
		["_default"] = "Settings",
		["Khariboli"] = "Settings",
	},

	["City Name"] = {
		["_default"] = "City Name",
		["Khariboli"] = "City Name",
	},

	["Language"] = {
		["_default"] = "Language",
		["Khariboli"] = "Language",
	},

	["Mute"] = {
		["_default"] = "Mute",
		["Khariboli"] = "Mute",
	},

	["Skip Song"] = {
		["_default"] = "Skip Song",
		["Khariboli"] = "Skip Song",
	},

	["Social Media"] = {
		["_default"] = "Social Media",
		["Khariboli"] = "Social Media",
	},

	["ConfirmDelete"] = {
		["_default"] = "ConfirmDelete",
		["Khariboli"] = "ConfirmDelete",
	},

	["Refund_Amount"] = {
		["_default"] = "Refund_Amount",
		["Khariboli"] = "Refund_Amount",
	},

	["Cant overlap zones"] = {
		["_default"] = "You can't overlap zones EASDASD KEY.",
		["Khariboli"] = "You can't overlap zones EASDASD KEY.",
	},

	["Cant build on water"] = {
		["_default"] = "You can't place that on water. Build a road first.",
		["Khariboli"] = "You can't place that on water. Build a road first.",
	},

	["Cant build on roads"] = {
		["_default"] = "You can't build on roads KEY.",
		["Khariboli"] = "You can't build on roads KEY.",
	},

	["Cant build on unique buildings"] = {
		["_default"] = "That area is reserved for a unique building KEY.",
		["Khariboli"] = "That area is reserved for a unique building KEY.",
	},

	["OB1_Begin"] = {
		["_default"] = "Let's build your first town. Follow the steps and watch the arrows.",
		["Khariboli"] = "Let's build your first town. Follow the steps and watch the arrows.",
	},

	["OB1_Complete"] = {
		["_default"] = "Great work! Your starter town has water and power.",
		["Khariboli"] = "Great work! Your starter town has water and power.",
	},

	["OB1_S1_Road_Hint"] = {
		["_default"] = "Select Road and draw a straight road to start your main street.",
		["Khariboli"] = "Select Road and draw a straight road to start your main street.",
	},

	["OB1_S1_Road_Done"] = {
		["_default"] = "Road placed!",
		["Khariboli"] = "Road placed!",
	},

	["OB1_S2_Residential_Hint"] = {
		["_default"] = "Zone Residential along the road. Drag a rectangle to place homes.",
		["Khariboli"] = "Zone Residential along the road. Drag a rectangle to place homes.",
	},

	["OB1_S2_Residential_Done"] = {
		["_default"] = "Homes are zoned.",
		["Khariboli"] = "Homes are zoned.",
	},

	["OB1_S3_WaterTower_Hint"] = {
		["_default"] = "Place a Water Tower to produce water for your city.",
		["Khariboli"] = "Place a Water Tower to produce water for your city.",
	},

	["OB1_S3_WaterTower_Done"] = {
		["_default"] = "Water production online.",
		["Khariboli"] = "Water production online.",
	},

	["OB1_S4_WaterPipe_A_Hint"] = {
		["_default"] = "Lay a water pipe from the tower toward the homes.",
		["Khariboli"] = "Lay a water pipe from the tower toward the homes.",
	},

	["OB1_S4_WaterPipe_A_Done"] = {
		["_default"] = "Pipe placed.",
		["Khariboli"] = "Pipe placed.",
	},

	["OB1_S5_WaterPipe_B_Hint"] = {
		["_default"] = "Run a pipe down so every house can connect.",
		["Khariboli"] = "Run a pipe down so every house can connect.",
	},

	["OB1_S5_WaterPipe_B_Done"] = {
		["_default"] = "Neighborhood connected to water.",
		["Khariboli"] = "Neighborhood connected to water.",
	},

	["OB1_S6_WindTurbine_Hint"] = {
		["_default"] = "Place a Wind Turbine to produce power.",
		["Khariboli"] = "Place a Wind Turbine to produce power.",
	},

	["OB1_S6_WindTurbine_Done"] = {
		["_default"] = "Power production online.",
		["Khariboli"] = "Power production online.",
	},

	["OB1_S7_PowerLines_A_Hint"] = {
		["_default"] = "Stretch power lines from the turbine toward the homes.",
		["Khariboli"] = "Stretch power lines from the turbine toward the homes.",
	},

	["OB1_S7_PowerLines_A_Done"] = {
		["_default"] = "Line connected.",
		["Khariboli"] = "Line connected.",
	},

	["OB1_S8_PowerLines_B_Hint"] = {
		["_default"] = "Pull a line down so all blocks get power.",
		["Khariboli"] = "Pull a line down so all blocks get power.",
	},

	["OB1_S8_PowerLines_B_Done"] = {
		["_default"] = "Neighborhood connected to power.",
		["Khariboli"] = "Neighborhood connected to power.",
	},

	["OB1_S9_Road2_Hint"] = {
		["_default"] = "Add another short road to expand your grid.",
		["Khariboli"] = "Add another short road to expand your grid.",
	},

	["OB1_S9_Road2_Done"] = {
		["_default"] = "Grid expanded.",
		["Khariboli"] = "Grid expanded.",
	},

	["OB1_S10_Commercial_Hint"] = {
		["_default"] = "Zone a Commercial area near your road for shops.",
		["Khariboli"] = "Zone a Commercial area near your road for shops.",
	},

	["OB1_S10_Commercial_Done"] = {
		["_default"] = "Shops are zoned.",
		["Khariboli"] = "Shops are zoned.",
	},

	["OB_OpenBuildMenu"] = {
		["_default"] = "Open the Build Menu to continue.",
		["Khariboli"] = "Open the Build Menu to continue.",
	},

	["OB_OpenTransportTab"] = {
		["_default"] = "Open the Transport tab.",
		["Khariboli"] = "Open the Transport tab.",
	},

	["OB_OpenZonesTab"] = {
		["_default"] = "Open the Zones tab.",
		["Khariboli"] = "Open the Zones tab.",
	},

	["OB_OpenSupplyTab"] = {
		["_default"] = "Open the Supply tab.",
		["Khariboli"] = "Open the Supply tab.",
	},

	["OB_OpenServicesTab"] = {
		["_default"] = "Open the Services tab.",
		["Khariboli"] = "Open the Services tab.",
	},

	["OB_OpenWaterHub"] = {
		["_default"] = "Choose the Water hub.",
		["Khariboli"] = "Choose the Water hub.",
	},

	["OB_OpenPowerHub"] = {
		["_default"] = "Choose the Power hub.",
		["Khariboli"] = "Choose the Power hub.",
	},

	["OB2_Begin"] = {
		["_default"] = "Utilities 101: keep buildings supplied with Water and Power.",
		["Khariboli"] = "Utilities 101: keep buildings supplied with Water and Power.",
	},

	["OB2_Complete"] = {
		["_default"] = "Utilities connected. Keep expanding!",
		["Khariboli"] = "Utilities connected. Keep expanding!",
	},

	["OB2_WaterDeficit"] = {
		["_default"] = "Water deficit: add or upgrade water production.",
		["Khariboli"] = "Water deficit: add or upgrade water production.",
	},

	["OB2_ConnectWater"] = {
		["_default"] = "Connect buildings to water with pipes.",
		["Khariboli"] = "Connect buildings to water with pipes.",
	},

	["OB2_PowerDeficit"] = {
		["_default"] = "Power deficit: add or upgrade power production.",
		["Khariboli"] = "Power deficit: add or upgrade power production.",
	},

	["OB2_ConnectPower"] = {
		["_default"] = "Connect buildings to power with lines.",
		["Khariboli"] = "Connect buildings to power with lines.",
	},

	["OB_SelectRoad"] = {
		["_default"] = "Select the Road tool.",
		["Khariboli"] = "Select the Road tool.",
	},

	["OB_SelectRoad_Done"] = {
		["_default"] = "Road tool selected.",
		["Khariboli"] = "Road tool selected.",
	},

	["OB3_Begin"] = {
		["_default"] = "Next: Place an Industrial zone and connect it to road, water, and power.",
		["Khariboli"] = "Next: Place an Industrial zone and connect it to road, water, and power.",
	},

	["OB3_Industrial_Hint"] = {
		["_default"] = "Drag to zone the highlighted area as Industrial.",
		["Khariboli"] = "Drag to zone the highlighted area as Industrial.",
	},

	["OB3_Industrial_Done"] = {
		["_default"] = "Industrial zone placed!",
		["Khariboli"] = "Industrial zone placed!",
	},

	["OB3_ConnectRoad"] = {
		["_default"] = "Connect the zone to a road.",
		["Khariboli"] = "Connect the zone to a road.",
	},

	["OB3_ConnectRoadNetwork"] = {
		["_default"] = "Connect that road to your city's road network.",
		["Khariboli"] = "Connect that road to your city's road network.",
	},

	["OB3_ConnectWater"] = {
		["_default"] = "Lay pipes to supply water to the zone.",
		["Khariboli"] = "Lay pipes to supply water to the zone.",
	},

	["OB3_ConnectPower"] = {
		["_default"] = "Run power lines to supply electricity to the zone.",
		["Khariboli"] = "Run power lines to supply electricity to the zone.",
	},

	["OB3_Complete"] = {
		["_default"] = "Great! The Industrial zone is fully connected.",
		["Khariboli"] = "Great! The Industrial zone is fully connected.",
	},

	["LOAD_LoadingCity"] = {
		["_default"] = "LOAD_LoadingCity",
		["Khariboli"] = "LOAD_LoadingCity",
	},

	["LOAD_Preparing"] = {
		["_default"] = "LOAD_Preparing",
		["Khariboli"] = "LOAD_Preparing",
	},

	["LOAD_SettingUpZones"] = {
		["_default"] = "LOAD_SettingUpZones",
		["Khariboli"] = "LOAD_SettingUpZones",
	},

	["LOAD_BuildingRoads"] = {
		["_default"] = "LOAD_BuildingRoads",
		["Khariboli"] = "LOAD_BuildingRoads",
	},

	["LOAD_PlacingBuildings"] = {
		["_default"] = "LOAD_PlacingBuildings",
		["Khariboli"] = "LOAD_PlacingBuildings",
	},

	["LOAD_LayingUtilities"] = {
		["_default"] = "LOAD_LayingUtilities",
		["Khariboli"] = "LOAD_LayingUtilities",
	},

	["LOAD_Finalizing"] = {
		["_default"] = "LOAD_Finalizing",
		["Khariboli"] = "LOAD_Finalizing",
	},

	["LOAD_LoadingSave"] = {
		["_default"] = "LOAD_LoadingSave",
		["Khariboli"] = "LOAD_LoadingSave",
	},

	["LOAD_SwitchingSave"] = {
		["_default"] = "LOAD_SwitchingSave",
		["Khariboli"] = "LOAD_SwitchingSave",
	},

	["LOAD_DeletingSave"] = {
		["_default"] = "LOAD_DeletingSave",
		["Khariboli"] = "LOAD_DeletingSave",
	},

	["Flag Afghanistan"] = {
		["_default"] = "Flag Afghanistan",
		["Khariboli"] = "Flag Afghanistan",
	},

	["Flag Albania"] = {
		["_default"] = "Flag Albania",
		["Khariboli"] = "Flag Albania",
	},

	["Flag Algeria"] = {
		["_default"] = "Flag Algeria",
		["Khariboli"] = "Flag Algeria",
	},

	["Flag America"] = {
		["_default"] = "Flag America",
		["Khariboli"] = "Flag America",
	},

	["Flag Angola"] = {
		["_default"] = "Flag Angola",
		["Khariboli"] = "Flag Angola",
	},

	["Flag Argentina"] = {
		["_default"] = "Flag Argentina",
		["Khariboli"] = "Flag Argentina",
	},

	["Flag Armenia"] = {
		["_default"] = "Flag Armenia",
		["Khariboli"] = "Flag Armenia",
	},

	["Flag Australia"] = {
		["_default"] = "Flag Australia",
		["Khariboli"] = "Flag Australia",
	},

	["Flag Austria"] = {
		["_default"] = "Flag Austria",
		["Khariboli"] = "Flag Austria",
	},

	["Flag Azerbaijan"] = {
		["_default"] = "Flag Azerbaijan",
		["Khariboli"] = "Flag Azerbaijan",
	},

	["Flag Bahrain"] = {
		["_default"] = "Flag Bahrain",
		["Khariboli"] = "Flag Bahrain",
	},

	["Flag Bangladesh"] = {
		["_default"] = "Flag Bangladesh",
		["Khariboli"] = "Flag Bangladesh",
	},

	["Flag Belarus"] = {
		["_default"] = "Flag Belarus",
		["Khariboli"] = "Flag Belarus",
	},

	["Flag Belgium"] = {
		["_default"] = "Flag Belgium",
		["Khariboli"] = "Flag Belgium",
	},

	["Flag Belize"] = {
		["_default"] = "Flag Belize",
		["Khariboli"] = "Flag Belize",
	},

	["Flag Benin"] = {
		["_default"] = "Flag Benin",
		["Khariboli"] = "Flag Benin",
	},

	["Flag Bhutanica"] = {
		["_default"] = "Flag Bhutanica",
		["Khariboli"] = "Flag Bhutanica",
	},

	["Flag Bolivia"] = {
		["_default"] = "Flag Bolivia",
		["Khariboli"] = "Flag Bolivia",
	},

	["Flag Bosnia"] = {
		["_default"] = "Flag Bosnia",
		["Khariboli"] = "Flag Bosnia",
	},

	["Flag Botswana"] = {
		["_default"] = "Flag Botswana",
		["Khariboli"] = "Flag Botswana",
	},

	["Flag Brazil"] = {
		["_default"] = "Flag Brazil",
		["Khariboli"] = "Flag Brazil",
	},

	["Flag Bulgaria"] = {
		["_default"] = "Flag Bulgaria",
		["Khariboli"] = "Flag Bulgaria",
	},

	["Flag Burkina Faso"] = {
		["_default"] = "Flag Burkina Faso",
		["Khariboli"] = "Flag Burkina Faso",
	},

	["Flag Burundi"] = {
		["_default"] = "Flag Burundi",
		["Khariboli"] = "Flag Burundi",
	},

	["Flag Cambodia"] = {
		["_default"] = "Flag Cambodia",
		["Khariboli"] = "Flag Cambodia",
	},

	["Flag Cameroon"] = {
		["_default"] = "Flag Cameroon",
		["Khariboli"] = "Flag Cameroon",
	},

	["Flag Canada"] = {
		["_default"] = "Flag Canada",
		["Khariboli"] = "Flag Canada",
	},

	["Flag Chad"] = {
		["_default"] = "Flag Chad",
		["Khariboli"] = "Flag Chad",
	},

	["Flag Chile"] = {
		["_default"] = "Flag Chile",
		["Khariboli"] = "Flag Chile",
	},

	["Flag China"] = {
		["_default"] = "Flag China",
		["Khariboli"] = "Flag China",
	},

	["Flag Colombia"] = {
		["_default"] = "Flag Colombia",
		["Khariboli"] = "Flag Colombia",
	},

	["Flag Congo"] = {
		["_default"] = "Flag Congo",
		["Khariboli"] = "Flag Congo",
	},

	["Flag Costa Rica"] = {
		["_default"] = "Flag Costa Rica",
		["Khariboli"] = "Flag Costa Rica",
	},

	["Flag Croatia"] = {
		["_default"] = "Flag Croatia",
		["Khariboli"] = "Flag Croatia",
	},

	["Flag Cuba"] = {
		["_default"] = "Flag Cuba",
		["Khariboli"] = "Flag Cuba",
	},

	["Flag Czech"] = {
		["_default"] = "Flag Czech",
		["Khariboli"] = "Flag Czech",
	},

	["Flag DR Congo"] = {
		["_default"] = "Flag DR Congo",
		["Khariboli"] = "Flag DR Congo",
	},

	["Flag Denmark"] = {
		["_default"] = "Flag Denmark",
		["Khariboli"] = "Flag Denmark",
	},

	["Flag Dominican Republic"] = {
		["_default"] = "Flag Dominican Republic",
		["Khariboli"] = "Flag Dominican Republic",
	},

	["Flag Ecuador"] = {
		["_default"] = "Flag Ecuador",
		["Khariboli"] = "Flag Ecuador",
	},

	["Flag Egypt"] = {
		["_default"] = "Flag Egypt",
		["Khariboli"] = "Flag Egypt",
	},

	["Flag El Salvador"] = {
		["_default"] = "Flag El Salvador",
		["Khariboli"] = "Flag El Salvador",
	},

	["Flag Eritrea"] = {
		["_default"] = "Flag Eritrea",
		["Khariboli"] = "Flag Eritrea",
	},

	["Flag Estonia"] = {
		["_default"] = "Flag Estonia",
		["Khariboli"] = "Flag Estonia",
	},

	["Flag Ethiopia"] = {
		["_default"] = "Flag Ethiopia",
		["Khariboli"] = "Flag Ethiopia",
	},

	["Flag Finland"] = {
		["_default"] = "Flag Finland",
		["Khariboli"] = "Flag Finland",
	},

	["Flag France"] = {
		["_default"] = "Flag France",
		["Khariboli"] = "Flag France",
	},

	["Flag Gabon"] = {
		["_default"] = "Flag Gabon",
		["Khariboli"] = "Flag Gabon",
	},

	["Flag Gambia"] = {
		["_default"] = "Flag Gambia",
		["Khariboli"] = "Flag Gambia",
	},

	["Flag Georgia"] = {
		["_default"] = "Flag Georgia",
		["Khariboli"] = "Flag Georgia",
	},

	["Flag Germany"] = {
		["_default"] = "Flag Germany",
		["Khariboli"] = "Flag Germany",
	},

	["Flag Ghana"] = {
		["_default"] = "Flag Ghana",
		["Khariboli"] = "Flag Ghana",
	},

	["Flag Greece"] = {
		["_default"] = "Flag Greece",
		["Khariboli"] = "Flag Greece",
	},

	["Flag Guatemala"] = {
		["_default"] = "Flag Guatemala",
		["Khariboli"] = "Flag Guatemala",
	},

	["Flag Guinea"] = {
		["_default"] = "Flag Guinea",
		["Khariboli"] = "Flag Guinea",
	},

	["Flag Honduras"] = {
		["_default"] = "Flag Honduras",
		["Khariboli"] = "Flag Honduras",
	},

	["Flag Hungary"] = {
		["_default"] = "Flag Hungary",
		["Khariboli"] = "Flag Hungary",
	},

	["Flag Iceland"] = {
		["_default"] = "Flag Iceland",
		["Khariboli"] = "Flag Iceland",
	},

	["Flag India"] = {
		["_default"] = "Flag India",
		["Khariboli"] = "Flag India",
	},

	["Flag Indonesia"] = {
		["_default"] = "Flag Indonesia",
		["Khariboli"] = "Flag Indonesia",
	},

	["Flag Iran"] = {
		["_default"] = "Flag Iran",
		["Khariboli"] = "Flag Iran",
	},

	["Flag Iraq"] = {
		["_default"] = "Flag Iraq",
		["Khariboli"] = "Flag Iraq",
	},

	["Flag Ireland"] = {
		["_default"] = "Flag Ireland",
		["Khariboli"] = "Flag Ireland",
	},

	["Flag Italy"] = {
		["_default"] = "Flag Italy",
		["Khariboli"] = "Flag Italy",
	},

	["Flag Ivory Coast"] = {
		["_default"] = "Flag Ivory Coast",
		["Khariboli"] = "Flag Ivory Coast",
	},

	["Flag Jamaica"] = {
		["_default"] = "Flag Jamaica",
		["Khariboli"] = "Flag Jamaica",
	},

	["Flag Japan"] = {
		["_default"] = "Flag Japan",
		["Khariboli"] = "Flag Japan",
	},

	["Flag Jordan"] = {
		["_default"] = "Flag Jordan",
		["Khariboli"] = "Flag Jordan",
	},

	["Flag Kazakhstan"] = {
		["_default"] = "Flag Kazakhstan",
		["Khariboli"] = "Flag Kazakhstan",
	},

	["Flag Kenya"] = {
		["_default"] = "Flag Kenya",
		["Khariboli"] = "Flag Kenya",
	},

	["Flag Kyrgyzstan"] = {
		["_default"] = "Flag Kyrgyzstan",
		["Khariboli"] = "Flag Kyrgyzstan",
	},

	["Flag Laos"] = {
		["_default"] = "Flag Laos",
		["Khariboli"] = "Flag Laos",
	},

	["Flag Latvia"] = {
		["_default"] = "Flag Latvia",
		["Khariboli"] = "Flag Latvia",
	},

	["Flag Lebanon"] = {
		["_default"] = "Flag Lebanon",
		["Khariboli"] = "Flag Lebanon",
	},

	["Flag Liberia"] = {
		["_default"] = "Flag Liberia",
		["Khariboli"] = "Flag Liberia",
	},

	["Flag Libya"] = {
		["_default"] = "Flag Libya",
		["Khariboli"] = "Flag Libya",
	},

	["Flag Lithuania"] = {
		["_default"] = "Flag Lithuania",
		["Khariboli"] = "Flag Lithuania",
	},

	["Flag Malawi"] = {
		["_default"] = "Flag Malawi",
		["Khariboli"] = "Flag Malawi",
	},

	["Flag Malaysia"] = {
		["_default"] = "Flag Malaysia",
		["Khariboli"] = "Flag Malaysia",
	},

	["Flag Maldova"] = {
		["_default"] = "Flag Maldova",
		["Khariboli"] = "Flag Maldova",
	},

	["Flag Mali"] = {
		["_default"] = "Flag Mali",
		["Khariboli"] = "Flag Mali",
	},

	["Flag Mauritania"] = {
		["_default"] = "Flag Mauritania",
		["Khariboli"] = "Flag Mauritania",
	},

	["Flag Mexico"] = {
		["_default"] = "Flag Mexico",
		["Khariboli"] = "Flag Mexico",
	},

	["Flag Mongolia"] = {
		["_default"] = "Flag Mongolia",
		["Khariboli"] = "Flag Mongolia",
	},

	["Flag Montenegro"] = {
		["_default"] = "Flag Montenegro",
		["Khariboli"] = "Flag Montenegro",
	},

	["Flag Morocco"] = {
		["_default"] = "Flag Morocco",
		["Khariboli"] = "Flag Morocco",
	},

	["Flag Mozambique"] = {
		["_default"] = "Flag Mozambique",
		["Khariboli"] = "Flag Mozambique",
	},

	["Flag Myanmar"] = {
		["_default"] = "Flag Myanmar",
		["Khariboli"] = "Flag Myanmar",
	},

	["Flag Namibia"] = {
		["_default"] = "Flag Namibia",
		["Khariboli"] = "Flag Namibia",
	},

	["Flag Netherlands"] = {
		["_default"] = "Flag Netherlands",
		["Khariboli"] = "Flag Netherlands",
	},

	["Flag New Zealand"] = {
		["_default"] = "Flag New Zealand",
		["Khariboli"] = "Flag New Zealand",
	},

	["Flag Nicaragua"] = {
		["_default"] = "Flag Nicaragua",
		["Khariboli"] = "Flag Nicaragua",
	},

	["Flag Nigeria"] = {
		["_default"] = "Flag Nigeria",
		["Khariboli"] = "Flag Nigeria",
	},

	["Flag Norway"] = {
		["_default"] = "Flag Norway",
		["Khariboli"] = "Flag Norway",
	},

	["Flag Oman"] = {
		["_default"] = "Flag Oman",
		["Khariboli"] = "Flag Oman",
	},

	["Flag Pakistan"] = {
		["_default"] = "Flag Pakistan",
		["Khariboli"] = "Flag Pakistan",
	},

	["Flag Palestine"] = {
		["_default"] = "Flag Palestine",
		["Khariboli"] = "Flag Palestine",
	},

	["Flag Panama"] = {
		["_default"] = "Flag Panama",
		["Khariboli"] = "Flag Panama",
	},

	["Flag Papua New Guinea"] = {
		["_default"] = "Flag Papua New Guinea",
		["Khariboli"] = "Flag Papua New Guinea",
	},

	["Flag Paraguay"] = {
		["_default"] = "Flag Paraguay",
		["Khariboli"] = "Flag Paraguay",
	},

	["Flag Peru"] = {
		["_default"] = "Flag Peru",
		["Khariboli"] = "Flag Peru",
	},

	["Flag Philippines"] = {
		["_default"] = "Flag Philippines",
		["Khariboli"] = "Flag Philippines",
	},

	["Flag Poland"] = {
		["_default"] = "Flag Poland",
		["Khariboli"] = "Flag Poland",
	},

	["Flag Qatar"] = {
		["_default"] = "Flag Qatar",
		["Khariboli"] = "Flag Qatar",
	},

	["Flag Romania"] = {
		["_default"] = "Flag Romania",
		["Khariboli"] = "Flag Romania",
	},

	["Flag Russia"] = {
		["_default"] = "Flag Russia",
		["Khariboli"] = "Flag Russia",
	},

	["Flag Rwanda"] = {
		["_default"] = "Flag Rwanda",
		["Khariboli"] = "Flag Rwanda",
	},

	["Flag Saudi Arabia"] = {
		["_default"] = "Flag Saudi Arabia",
		["Khariboli"] = "Flag Saudi Arabia",
	},

	["Flag Senegal"] = {
		["_default"] = "Flag Senegal",
		["Khariboli"] = "Flag Senegal",
	},

	["Flag Serbia"] = {
		["_default"] = "Flag Serbia",
		["Khariboli"] = "Flag Serbia",
	},

	["Flag Sierra Leone"] = {
		["_default"] = "Flag Sierra Leone",
		["Khariboli"] = "Flag Sierra Leone",
	},

	["Flag Singapore"] = {
		["_default"] = "Flag Singapore",
		["Khariboli"] = "Flag Singapore",
	},

	["Flag Slovakia"] = {
		["_default"] = "Flag Slovakia",
		["Khariboli"] = "Flag Slovakia",
	},

	["Flag Somalia"] = {
		["_default"] = "Flag Somalia",
		["Khariboli"] = "Flag Somalia",
	},

	["Flag South Africa"] = {
		["_default"] = "Flag South Africa",
		["Khariboli"] = "Flag South Africa",
	},

	["Flag South Korea"] = {
		["_default"] = "Flag South Korea",
		["Khariboli"] = "Flag South Korea",
	},

	["Flag South Sudan"] = {
		["_default"] = "Flag South Sudan",
		["Khariboli"] = "Flag South Sudan",
	},

	["Flag Spain"] = {
		["_default"] = "Flag Spain",
		["Khariboli"] = "Flag Spain",
	},

	["Flag Sri Lanka"] = {
		["_default"] = "Flag Sri Lanka",
		["Khariboli"] = "Flag Sri Lanka",
	},

	["Flag Sudan"] = {
		["_default"] = "Flag Sudan",
		["Khariboli"] = "Flag Sudan",
	},

	["Flag Sweden"] = {
		["_default"] = "Flag Sweden",
		["Khariboli"] = "Flag Sweden",
	},

	["Flag Switzerland"] = {
		["_default"] = "Flag Switzerland",
		["Khariboli"] = "Flag Switzerland",
	},

	["Flag Syria"] = {
		["_default"] = "Flag Syria",
		["Khariboli"] = "Flag Syria",
	},

	["Flag Taiwan"] = {
		["_default"] = "Flag Taiwan",
		["Khariboli"] = "Flag Taiwan",
	},

	["Flag Tajikistan"] = {
		["_default"] = "Flag Tajikistan",
		["Khariboli"] = "Flag Tajikistan",
	},

	["Flag Tanzania"] = {
		["_default"] = "Flag Tanzania",
		["Khariboli"] = "Flag Tanzania",
	},

	["Flag Thailand"] = {
		["_default"] = "Flag Thailand",
		["Khariboli"] = "Flag Thailand",
	},

	["Flag Togo"] = {
		["_default"] = "Flag Togo",
		["Khariboli"] = "Flag Togo",
	},

	["Flag Tunisia"] = {
		["_default"] = "Flag Tunisia",
		["Khariboli"] = "Flag Tunisia",
	},

	["Flag Turkmenistan"] = {
		["_default"] = "Flag Turkmenistan",
		["Khariboli"] = "Flag Turkmenistan",
	},

	["Flag Türkiye"] = {
		["_default"] = "Flag Türkiye",
		["Khariboli"] = "Flag Türkiye",
	},

	["Flag Uganda"] = {
		["_default"] = "Flag Uganda",
		["Khariboli"] = "Flag Uganda",
	},

	["Flag Ukraine"] = {
		["_default"] = "Flag Ukraine",
		["Khariboli"] = "Flag Ukraine",
	},

	["Flag United Kingdom"] = {
		["_default"] = "Flag United Kingdom",
		["Khariboli"] = "Flag United Kingdom",
	},

	["Flag United Nations"] = {
		["_default"] = "Flag United Nations",
		["Khariboli"] = "Flag United Nations",
	},

	["Flag Uruguay"] = {
		["_default"] = "Flag Uruguay",
		["Khariboli"] = "Flag Uruguay",
	},

	["Flag Uzbekistan"] = {
		["_default"] = "Flag Uzbekistan",
		["Khariboli"] = "Flag Uzbekistan",
	},

	["Flag Venezuela"] = {
		["_default"] = "Flag Venezuela",
		["Khariboli"] = "Flag Venezuela",
	},

	["Flag Vietnam"] = {
		["_default"] = "Flag Vietnam",
		["Khariboli"] = "Flag Vietnam",
	},

	["Flag Yemen"] = {
		["_default"] = "Flag Yemen",
		["Khariboli"] = "Flag Yemen",
	},

	["Flag Zambia"] = {
		["_default"] = "Flag Zambia",
		["Khariboli"] = "Flag Zambia",
	},

	["Flag Zimbabwe"] = {
		["_default"] = "Flag Zimbabwe",
		["Khariboli"] = "Flag Zimbabwe",
	},

	["Build"] = {
		["_default"] = "Build",
		["Khariboli"] = "निर्माण",
	},

	["Home"] = {
		["_default"] = "Home",
		["Khariboli"] = "घर",
	},

	["Undo"] = {
		["_default"] = "Undo",
		["Khariboli"] = "पूर्ववत करें",
	},

	["Redo"] = {
		["_default"] = "Redo",
		["Khariboli"] = "फिर से करें",
	},

	["Transport"] = {
		["_default"] = "Transport",
		["Khariboli"] = "परिवहन",
	},

	["Zones"] = {
		["_default"] = "Zones",
		["Khariboli"] = "क्षेत्र",
	},

	["Services"] = {
		["_default"] = "Services",
		["Khariboli"] = "सेवाएँ",
	},

	["Supply"] = {
		["_default"] = "Supply",
		["Khariboli"] = "आपूर्ति",
	},

	["Road"] = {
		["_default"] = "Road",
		["Khariboli"] = "सड़क",
	},

	["Bus"] = {
		["_default"] = "Bus",
		["Khariboli"] = "बस",
	},

	["Metro"] = {
		["_default"] = "Metro",
		["Khariboli"] = "मेट्रो",
	},

	["Airport"] = {
		["_default"] = "Airport",
		["Khariboli"] = "हवाई अड्डा",
	},

	["Airport Owned"] = {
		["_default"] = "Airport (Owned)",
		["Khariboli"] = "Airport (Owned)",
	},

	["Residential Zone"] = {
		["_default"] = "Residential Zone",
		["Khariboli"] = "आवासीय क्षेत्र",
	},

	["Commercial Zone"] = {
		["_default"] = "Commercial Zone",
		["Khariboli"] = "व्यावसायिक क्षेत्र",
	},

	["Industrial Zone"] = {
		["_default"] = "Industrial Zone",
		["Khariboli"] = "औद्योगिक क्षेत्र",
	},

	["Dense Residential Zone"] = {
		["_default"] = "Dense Residential Zone",
		["Khariboli"] = "घने आवासीय क्षेत्र",
	},

	["Dense Commercial Zone"] = {
		["_default"] = "Dense Commercial Zone",
		["Khariboli"] = "घना व्यावसायिक क्षेत्र",
	},

	["Dense Industrial Zone"] = {
		["_default"] = "Dense Industrial Zone",
		["Khariboli"] = "घना औद्योगिक क्षेत्र",
	},

	["Residential"] = {
		["_default"] = "Residential",
		["Khariboli"] = "आवासीय",
	},

	["Commercial"] = {
		["_default"] = "Commercial",
		["Khariboli"] = "व्यावसायिक",
	},

	["Industrial"] = {
		["_default"] = "Industrial",
		["Khariboli"] = "औद्योगिक",
	},

	["Fire Precinct"] = {
		["_default"] = "Fire Precinct",
		["Khariboli"] = "अग्निशमन क्षेत्र",
	},

	["Fire Station"] = {
		["_default"] = "Fire Station",
		["Khariboli"] = "दमकल स्टेशन",
	},

	["City Hospital"] = {
		["_default"] = "City Hospital",
		["Khariboli"] = "शहर अस्पताल",
	},

	["Local Hospital"] = {
		["_default"] = "Local Hospital",
		["Khariboli"] = "स्थानीय अस्पताल",
	},

	["Major Hospital"] = {
		["_default"] = "Major Hospital",
		["Khariboli"] = "मुख्य अस्पताल",
	},

	["Small Clinic"] = {
		["_default"] = "Small Clinic",
		["Khariboli"] = "छोटी क्लिनिक",
	},

	["MiddleSchool"] = {
		["_default"] = "Middle School",
		["Khariboli"] = "Middle School",
	},

	["Museum"] = {
		["_default"] = "Museum",
		["Khariboli"] = "संग्रहालय",
	},

	["NewsStation"] = {
		["_default"] = "News Station",
		["Khariboli"] = "News Station",
	},

	["PrivateSchool"] = {
		["_default"] = "Private School",
		["Khariboli"] = "Private School",
	},

	["Bus Depot"] = {
		["_default"] = "Bus Depot",
		["Khariboli"] = "बस डिपो",
	},

	["Bus Depot Owned"] = {
		["_default"] = "Bus Depot (Owned)",
		["Khariboli"] = "Bus Depot (Owned)",
	},

	["Handy-Hardware"] = {
		["_default"] = "Handy-Hardware",
		["Khariboli"] = "Handy-Hardware",
	},

	["Big-Buy-Mart"] = {
		["_default"] = "Big-Buy-Mart",
		["Khariboli"] = "Big-Buy-Mart",
	},

	["Loading..."] = {
		["_default"] = "Loading...",
		["Khariboli"] = "Loading...",
	},

	["Great-Grocery"] = {
		["_default"] = "Great-Grocery",
		["Khariboli"] = "Great-Grocery",
	},

	["Supermarket"] = {
		["_default"] = "Supermarket",
		["Khariboli"] = "Supermarket",
	},

	["Tea House"] = {
		["_default"] = "Tea House",
		["Khariboli"] = "Tea House",
	},

	["LAUNDROMAT"] = {
		["_default"] = "LAUNDROMAT",
		["Khariboli"] = "LAUNDROMAT",
	},

	["REPAIRS"] = {
		["_default"] = "REPAIRS",
		["Khariboli"] = "REPAIRS",
	},

	["GROCERY"] = {
		["_default"] = "GROCERY",
		["Khariboli"] = "GROCERY",
	},

	["Cozy Coffee"] = {
		["_default"] = "Cozy Coffee",
		["Khariboli"] = "Cozy Coffee",
	},

	["Icecream"] = {
		["_default"] = "Icecream",
		["Khariboli"] = "Icecream",
	},

	["Car Garage"] = {
		["_default"] = "Car Garage",
		["Khariboli"] = "Car Garage",
	},

	["Italian-To-Go"] = {
		["_default"] = "Italian-To-Go",
		["Khariboli"] = "Italian-To-Go",
	},

	["Corner Cafe"] = {
		["_default"] = "Corner Cafe",
		["Khariboli"] = "Corner Cafe",
	},

	["HAIR-SALON"] = {
		["_default"] = "HAIR-SALON",
		["Khariboli"] = "HAIR-SALON",
	},

	["CLEANERS"] = {
		["_default"] = "CLEANERS",
		["Khariboli"] = "CLEANERS",
	},

	["HARDWARE"] = {
		["_default"] = "HARDWARE",
		["Khariboli"] = "HARDWARE",
	},

	["Thea's Gym"] = {
		["_default"] = "Thea's Gym",
		["Khariboli"] = "Thea's Gym",
	},

	["Auto Repair"] = {
		["_default"] = "Auto Repair",
		["Khariboli"] = "Auto Repair",
	},

	["Tech Store"] = {
		["_default"] = "Tech Store",
		["Khariboli"] = "Tech Store",
	},

	["Book Store"] = {
		["_default"] = "Book Store",
		["Khariboli"] = "Book Store",
	},

	["Asian-Buffet"] = {
		["_default"] = "Asian-Buffet",
		["Khariboli"] = "Asian-Buffet",
	},

	["Bistro"] = {
		["_default"] = "Bistro",
		["Khariboli"] = "Bistro",
	},

	["Borgirs"] = {
		["_default"] = "Borgirs",
		["Khariboli"] = "Borgirs",
	},

	["Pizzeria"] = {
		["_default"] = "Pizzeria",
		["Khariboli"] = "Pizzeria",
	},

	["AIRPORT"] = {
		["_default"] = "AIRPORT",
		["Khariboli"] = "हवाई अड्डा",
	},

	["METRO"] = {
		["_default"] = "METRO",
		["Khariboli"] = "मेट्रो",
	},

	["BUS DEPOT"] = {
		["_default"] = "BUS DEPOT",
		["Khariboli"] = "BUS DEPOT",
	},

	["School"] = {
		["_default"] = "School",
		["Khariboli"] = "School",
	},

	["NEWS"] = {
		["_default"] = "NEWS",
		["Khariboli"] = "NEWS",
	},

	["FIRE STATION"] = {
		["_default"] = "FIRE STATION",
		["Khariboli"] = "FIRE STATION",
	},

	["HOSPITAL"] = {
		["_default"] = "HOSPITAL",
		["Khariboli"] = "HOSPITAL",
	},

	["DOCTOR"] = {
		["_default"] = "DOCTOR",
		["Khariboli"] = "DOCTOR",
	},

	["Courthouse"] = {
		["_default"] = "Courthouse",
		["Khariboli"] = "अदालत",
	},

	["POLICE"] = {
		["_default"] = "POLICE",
		["Khariboli"] = "POLICE",
	},

	["Bank"] = {
		["_default"] = "Bank",
		["Khariboli"] = "बैंक",
	},

	["CN Tower"] = {
		["_default"] = "CN Tower",
		["Khariboli"] = "सीएन टॉवर",
	},

	["Eiffel Tower"] = {
		["_default"] = "Eiffel Tower",
		["Khariboli"] = "आइफ़िल टॉवर",
	},

	["Empire State Building"] = {
		["_default"] = "Empire State Building",
		["Khariboli"] = "एंपायर स्टेट बिल्डिंग",
	},

	["Ferris Wheel"] = {
		["_default"] = "Ferris Wheel",
		["Khariboli"] = "फेरिस व्हील",
	},

	["Gas Station"] = {
		["_default"] = "Gas Station",
		["Khariboli"] = "गैस स्टेशन",
	},

	["Modern Skyscraper"] = {
		["_default"] = "Modern Skyscraper",
		["Khariboli"] = "आधुनिक गगनचुंबी इमारत",
	},

	["National Capital"] = {
		["_default"] = "National Capital",
		["Khariboli"] = "राष्ट्रीय राजधानी",
	},

	["Obelisk"] = {
		["_default"] = "Obelisk",
		["Khariboli"] = "ओबेलिस्क",
	},

	["Space Needle"] = {
		["_default"] = "Space Needle",
		["Khariboli"] = "स्पेस नीडल",
	},

	["Statue of Liberty"] = {
		["_default"] = "Statue of Liberty",
		["Khariboli"] = "स्वतंत्रता की प्रतिमा",
	},

	["Tech Office"] = {
		["_default"] = "Tech Office",
		["Khariboli"] = "टेक कार्यालय",
	},

	["World Trade Center"] = {
		["_default"] = "World Trade Center",
		["Khariboli"] = "वर्ल्ड ट्रेड सेंटर",
	},

	["Church"] = {
		["_default"] = "Church",
		["Khariboli"] = "गिरजाघर",
	},

	["Hotel"] = {
		["_default"] = "Hotel",
		["Khariboli"] = "होटल",
	},

	["Mosque"] = {
		["_default"] = "Mosque",
		["Khariboli"] = "मस्जिद",
	},

	["Movie Theater"] = {
		["_default"] = "Movie Theater",
		["Khariboli"] = "सिनेमा थिएटर",
	},

	["Shinto Temple"] = {
		["_default"] = "Shinto Jinja",
		["Khariboli"] = "शिन्तो जिनजा",
	},

	["Buddha Statue"] = {
		["_default"] = "Buddha Statue",
		["Khariboli"] = "बुद्ध प्रतिमा",
	},

	["Hindu Temple"] = {
		["_default"] = "Hindu Mandir",
		["Khariboli"] = "हिंदू मंदिर",
	},

	["Courthouse"] = {
		["_default"] = "Courthouse",
		["Khariboli"] = "अदालत",
	},

	["PoliceDept"] = {
		["_default"] = "Police Dept",
		["Khariboli"] = "Police Dept",
	},

	["PolicePrecinct"] = {
		["_default"] = "Police Precinct",
		["Khariboli"] = "Police Precinct",
	},

	["PoliceStation"] = {
		["_default"] = "Police Station",
		["Khariboli"] = "Police Station",
	},

	["Archery Range"] = {
		["_default"] = "Archery Range",
		["Khariboli"] = "तीरंदाजी मैदान",
	},

	["Basketball Court"] = {
		["_default"] = "Basketball Court",
		["Khariboli"] = "बास्केटबॉल कोर्ट",
	},

	["Basketball Stadium"] = {
		["_default"] = "Basketball Stadium",
		["Khariboli"] = "बास्केटबॉल स्टेडियम",
	},

	["Football Stadium"] = {
		["_default"] = "Football Stadium",
		["Khariboli"] = "फुटबॉल स्टेडियम",
	},

	["Golf Course"] = {
		["_default"] = "Golf Course",
		["Khariboli"] = "गोल्फ कोर्स",
	},

	["Public Pool"] = {
		["_default"] = "Public Pool",
		["Khariboli"] = "सार्वजनिक पूल",
	},

	["Skate Park"] = {
		["_default"] = "Skate Park",
		["Khariboli"] = "स्केट पार्क",
	},

	["Soccer Stadium"] = {
		["_default"] = "Soccer Stadium",
		["Khariboli"] = "सॉकर स्टेडियम",
	},

	["Tennis Court"] = {
		["_default"] = "Tennis Court",
		["Khariboli"] = "टेनिस कोर्ट",
	},

	["Coal Power Plant"] = {
		["_default"] = "Coal Power Plant",
		["Khariboli"] = "कोयला बिजली संयंत्र",
	},

	["Gas Power Plant"] = {
		["_default"] = "Gas Power Plant",
		["Khariboli"] = "गैस बिजली संयंत्र",
	},

	["Geothermal Power Plant"] = {
		["_default"] = "Geothermal Power Plant",
		["Khariboli"] = "भू-तापीय बिजली संयंत्र",
	},

	["Nuclear Power Plant"] = {
		["_default"] = "Nuclear Power Plant",
		["Khariboli"] = "परमाणु बिजली संयंत्र",
	},

	["Solar Panels"] = {
		["_default"] = "Solar Panels",
		["Khariboli"] = "सौर पैनल",
	},

	["Wind Turbine"] = {
		["_default"] = "Wind Turbine",
		["Khariboli"] = "पवन टर्बाइन",
	},

	["Power Lines"] = {
		["_default"] = "Power Line",
		["Khariboli"] = "Power Line",
	},

	["Water Tower"] = {
		["_default"] = "Water Tower",
		["Khariboli"] = "जल टॉवर",
	},

	["Water Plant"] = {
		["_default"] = "Water Plant",
		["Khariboli"] = "जल संयंत्र",
	},

	["Purification Water Plant"] = {
		["_default"] = "Water Purification Plant",
		["Khariboli"] = "जल शुद्धिकरण संयंत्र",
	},

	["MolecularWaterPlant"] = {
		["_default"] = "Molecular Water Plant",
		["Khariboli"] = "Molecular Water Plant",
	},

	["Water Pipes"] = {
		["_default"] = "Water Pipes",
		["Khariboli"] = "पानी के पाइप",
	},

	["Leisure"] = {
		["_default"] = "Leisure",
		["Khariboli"] = "मनोरंजन",
	},

	["Fire Dept"] = {
		["_default"] = "Fire Dept",
		["Khariboli"] = "दमकल विभाग",
	},

	["Fire Depth"] = {
		["_default"] = "Fire Dept",
		["Khariboli"] = "Fire Dept",
	},

	["Police"] = {
		["_default"] = "Police",
		["Khariboli"] = "पुलिस",
	},

	["Health"] = {
		["_default"] = "Health",
		["Khariboli"] = "स्वास्थ्य",
	},

	["Education"] = {
		["_default"] = "Education",
		["Khariboli"] = "शिक्षा",
	},

	["Sports"] = {
		["_default"] = "Sports",
		["Khariboli"] = "खेल",
	},

	["Landmarks"] = {
		["_default"] = "Landmarks",
		["Khariboli"] = "ऐतिहासिक स्थल",
	},

	["Power"] = {
		["_default"] = "Power",
		["Khariboli"] = "बिजली",
	},

	["Water"] = {
		["_default"] = "Water",
		["Khariboli"] = "पानी",
	},

	["Garbage"] = {
		["_default"] = "Garbage",
		["Khariboli"] = "कचरा",
	},

	["Graves"] = {
		["_default"] = "Graves",
		["Khariboli"] = "कब्रें",
	},

	["Co-Op"] = {
		["_default"] = "Co-Op",
		["Khariboli"] = "को-ऑप",
	},

	["You've been invited to co-op PlayerName"] = {
		["_default"] = "You've been invited to co-op PlayerName",
		["Khariboli"] = "आपको को-ऑप PlayerName के लिए आमंत्रित किया गया है",
	},

	["Ignore"] = {
		["_default"] = "Ignore",
		["Khariboli"] = "अनदेखा करें",
	},

	["Accept"] = {
		["_default"] = "Accept",
		["Khariboli"] = "स्वीकार करें",
	},

	["Invite Friends"] = {
		["_default"] = "Invite Friends",
		["Khariboli"] = "मित्रों को आमंत्रित करें",
	},

	["Leave Co-Op"] = {
		["_default"] = "Leave Co-Op",
		["Khariboli"] = "को-ऑप छोड़ें",
	},

	["Invite"] = {
		["_default"] = "Invite",
		["Khariboli"] = "आमंत्रित करें",
	},

	["Demands"] = {
		["_default"] = "Demands",
		["Khariboli"] = "मांगें",
	},

	["Demand = Higher Bar"] = {
		["_default"] = "Demand = Higher Bar",
		["Khariboli"] = "मांग = ऊंचा स्तर",
	},

	["Poor"] = {
		["_default"] = "Poor",
		["Khariboli"] = "गरीब",
	},

	["Medium"] = {
		["_default"] = "Medium",
		["Khariboli"] = "मध्यम",
	},

	["Wealthy"] = {
		["_default"] = "Wealthy",
		["Khariboli"] = "धनी",
	},

	["Normal"] = {
		["_default"] = "Normal",
		["Khariboli"] = "सामान्य",
	},

	["High Density"] = {
		["_default"] = "High Density",
		["Khariboli"] = "उच्च घनत्व",
	},

	["Produced WATER"] = {
		["_default"] = "Produced: %s L",
		["Khariboli"] = "उत्पादित: %s L",
	},

	["Used WATER"] = {
		["_default"] = "Used: %s L",
		["Khariboli"] = "इस्तेमाल: %s L",
	},

	["Usage WATER"] = {
		["_default"] = "Usage %s%%",
		["Khariboli"] = "उपयोग %s%%",
	},

	["Produced POWER"] = {
		["_default"] = "Produced: %s W",
		["Khariboli"] = "उत्पादित: %s W",
	},

	["Used POWER"] = {
		["_default"] = "Used: %s W",
		["Khariboli"] = "इस्तेमाल: %s W",
	},

	["Usage POWER"] = {
		["_default"] = "Usage %s%%",
		["Khariboli"] = "उपयोग %s%%",
	},

	["Build A City"] = {
		["_default"] = "Build A City",
		["Khariboli"] = "एक शहर बनाएं",
	},

	["Load"] = {
		["_default"] = "Load",
		["Khariboli"] = "लोड",
	},

	["New"] = {
		["_default"] = "New",
		["Khariboli"] = "नया",
	},

	["New City"] = {
		["_default"] = "New",
		["Khariboli"] = "नया शहर",
	},

	["Last Played"] = {
		["_default"] = "Last Played",
		["Khariboli"] = "अंतिम खेला गया",
	},

	["Money"] = {
		["_default"] = "Money",
		["Khariboli"] = "पैसा",
	},

	["Gamepass"] = {
		["_default"] = "Gamepass",
		["Khariboli"] = "गेमपास",
	},

	["10% More!"] = {
		["_default"] = "10% More!",
		["Khariboli"] = "10% अधिक!",
	},

	["15% More!"] = {
		["_default"] = "15% More!",
		["Khariboli"] = "15% अधिक!",
	},

	["25% More!"] = {
		["_default"] = "25% More!",
		["Khariboli"] = "25% अधिक!",
	},

	["Best Deal!"] = {
		["_default"] = "Best Deal!",
		["Khariboli"] = "सर्वश्रेष्ठ सौदा!",
	},

	["$"] = {
		["_default"] = "$",
		["Khariboli"] = "$",
	},

	["Purchase"] = {
		["_default"] = "Purchase",
		["Khariboli"] = "खरीदें",
	},

	["Ok"] = {
		["_default"] = "Ok",
		["Khariboli"] = "ठीक",
	},

	["Thank you!"] = {
		["_default"] = "Thank you!",
		["Khariboli"] = "धन्यवाद!",
	},

	["Thanks for supporting our team, and our charity donations :D"] = {
		["_default"] = "Thanks for supporting our team, and our charity donations :D",
		["Khariboli"] = "हमारी टीम और हमारे दान को समर्थन देने के लिए धन्यवाद :D",
	},

	["A blackout again? I can't even call or email the electrical company when there's no power :("] = {
		["_default"] = "A blackout again? I can't even call or email the electrical company when there's no power :(",
		["Khariboli"] = "फिर से बिजली कटौती? जब बिजली नहीं होती तो मैं विद्युत कंपनी को कॉल या ईमेल भी नहीं कर सकता :(",
		["Punjabi"] = "ਫਿਰ ਬਲੈਕਆਉਟ? ਜਦ ਬਿਜਲੀ ਨਹੀਂ ਹੁੰਦੀ ਤਾਂ ਮੈਂ ਬਿਜਲੀ ਕੰਪਨੀ ਨੂੰ ਕਾਲ ਜਾਂ ਈਮੇਲ ਵੀ ਨਹੀਂ ਕਰ ਸਕਦਾ :(",
		["Bhojpuri"] = "फेर से बिजली गुल? बिजली ना होखे त हम बिजली कंपनी के कॉलो/ईमेलो ना कर सकी :(",
		["Haryanvi"] = "फेर से ब्लैकआउट? बिजली न होवे तैं मैं बिजली कंपनी नै फोन या ईमेल भी ना कर सकूँ :(",
	},

	["Help, my TV doesn't work when there's no power!"] = {
		["_default"] = "Help, my TV doesn't work when there's no power!",
		["Khariboli"] = "मदद करो, बिजली नहीं होने पर मेरा टीवी काम नहीं करता!",
		["Punjabi"] = "ਮਦਦ ਕਰੋ, ਜਦ ਬਿਜਲੀ ਨਹੀਂ ਹੁੰਦੀ ਤਾਂ ਮੇਰਾ ਟੀਵੀ ਕੰਮ ਨਹੀਂ ਕਰਦਾ!",
		["Bhojpuri"] = "मदद, बिजली ना रहे त हमार टीवी ना चले!",
		["Haryanvi"] = "मदद करो, बिजली ना होवे तो मेरा टीवी ना चालै!",
	},

	["Power to the people! How hard is it to build a working power grid? It’s not exactly state of the art technology."] = {
		["_default"] = "Power to the people! How hard is it to build a working power grid? It’s not exactly state of the art technology.",
		["Khariboli"] = "लोगों को बिजली दो! एक काम कर रही बिजली ग्रिड बनाना कितना मुश्किल है? यह कोई अत्याधुनिक तकनीक नहीं है।",
		["Punjabi"] = "ਲੋਕਾਂ ਨੂੰ ਬਿਜਲੀ ਦਿਓ! ਚਲਦੀ ਬਿਜਲੀ ਗਰਿਡ ਬਣਾਉਣਾ ਕਿੰਨਾ ਔਖਾ ਹੈ? ਇਹ ਕੋਈ ਹਾਈ ਟੈਕ ਤਕਨਾਲੋਜੀ ਨਹੀਂ ਹੈ।",
		["Bhojpuri"] = "लोगन के बिजली चाहीं! काम करे वाला ग्रिड बनावल कतना मुश्किल बा? ई त कोई नया तकनीक ना ह।",
		["Haryanvi"] = "लोगां नै बिजली दे! चालू बिजली ग्रिड बनावण में कितनी मुश्किल सै? ये कोई नई तकनीक तो सै ना।",
	},

	["Hey, guys, is the water supposed to be brown and crunchy?"] = {
		["_default"] = "Hey, guys, is the water supposed to be brown and crunchy?",
		["Khariboli"] = "अरे दोस्तों, क्या पानी भूरा और कुरकुरा होना चाहिए?",
		["Punjabi"] = "ਹੇ ਦੋਸਤੋ, ਕੀ ਪਾਣੀ ਭੂਰਾ ਤੇ ਕੜਕ ਹੋਣਾ ਚਾਹੀਦਾ ਹੈ?",
		["Bhojpuri"] = "अरे, पानी त भूरा आ कुरकुरा होखे के चाहीं?",
		["Haryanvi"] = "अरे भाइयो, के पानी भूरा और करड़ा होणा चाहिए?",
	},

	["Don't I pay my taxes for services like water??? This is absurd!"] = {
		["_default"] = "Don't I pay my taxes for services like water??? This is absurd!",
		["Khariboli"] = "क्या मैं पानी जैसी सेवाओं के लिए टैक्स नहीं देता??? यह बेतुका है!",
		["Punjabi"] = "ਕੀ ਮੈਂ ਪਾਣੀ ਵਰਗੀਆਂ ਸੇਵਾਵਾਂ ਲਈ ਟੈਕਸ ਨਹੀਂ ਦੇਂਦਾ??? ਇਹ ਬੇਵਕੂਫੀ ਹੈ!",
		["Bhojpuri"] = "का हम पानी जइसन सेवा खातिर टैक्स ना देत हई??? ई बहुत बेतुका बा!",
		["Haryanvi"] = "मैं पानी जैसी सेवाओं का टैक्स न भरू? ये तो बेमतलब सै!",
	},

	["I would think that fresh water is basic stuff, but NO! How long do we have to wait for working water pipes!?"] = {
		["_default"] = "I would think that fresh water is basic stuff, but NO! How long do we have to wait for working water pipes!?",
		["Khariboli"] = "मैं तो सोचता था कि ताजा पानी बुनियादी चीज है, लेकिन नहीं! काम करने वाले पाइपों के लिए हमें कितना इंतजार करना पड़ेगा!?",
		["Punjabi"] = "ਮੈਂ ਸੋਚਦਾ ਸੀ ਕਿ ਤਾਜਾ ਪਾਣੀ ਮੂਲ ਭਾਲ਼ਾ ਹੈ, ਪਰ ਨਹੀਂ! ਚਲਦੀਆਂ ਪਾਈਪਾਂ ਲਈ ਸਾਨੂੰ ਕਿੰਨਾ ਉਡੀਕਣਾ ਪਵੇਗਾ?!",
		["Bhojpuri"] = "हम सोचत रहनी कि साफ पानी बुनियादी चीज ह, बाकि ना! चलत पानी पाइप खातिर कब तक इंतजार करी?!",
		["Haryanvi"] = "मैं सोचता था कि साफ पानी तो बुनियादी चीज सै, पर ना! काम करती पाइपां का कितनी देर तक इंतजार करना पड़ेगा?!",
	},

	["Ban crime! I want no more crime!"] = {
		["_default"] = "Ban crime! I want no more crime!",
		["Khariboli"] = "अपराध पर प्रतिबंध लगाओ! मैं और अपराध नहीं चाहता!",
		["Punjabi"] = "ਅਪਰਾਧ ਤੇ ਪਾਬੰਦੀ ਲਗਾਓ! ਮੈਂ ਹੋਰ ਅਪਰਾਧ ਨਹੀਂ ਚਾਹੁੰਦਾ!",
		["Bhojpuri"] = "अपराध बंद करऽ! हम अउर अपराध नइखे चाहीं!",
		["Haryanvi"] = "अपराध पर बैन लगाओ! मैं और अपराध ना चाहू!",
	},

	["I will happily pay some extra taxes, if we can get the crime levels down."] = {
		["_default"] = "I will happily pay some extra taxes, if we can get the crime levels down.",
		["Khariboli"] = "अगर हम अपराध के स्तर को कम कर सकें तो मैं खुशी से थोड़ा अतिरिक्त टैक्स दूँगा।",
		["Punjabi"] = "ਜੇ ਅਸੀਂ ਅਪਰਾਧ ਦਾ ਪੱਧਰ ਘਟਾ ਸਕੀਏ ਤਾਂ ਮੈਂ ਖੁਸ਼ੀ ਨਾਲ ਵਾਧੂ ਟੈਕਸ ਦਿਆਂਗਾ।",
		["Bhojpuri"] = "अगर अपराध कम होई त हम खुशी से कुछ अउर टैक्स देब।",
		["Haryanvi"] = "अगर हम अपराध कम कर सकें तो मैं खुशी-खुशी थोड़ा ज्यादा टैक्स दे दूंगा।",
	},

	["Every night it’s sirens and shouting. Where are the police?"] = {
		["_default"] = "Every night it’s sirens and shouting. Where are the police?",
		["Khariboli"] = "Every night it’s sirens and shouting. Where are the police?",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Break-ins everywhere. We need a police station closer than the next city over."] = {
		["_default"] = "Break-ins everywhere. We need a police station closer than the next city over.",
		["Khariboli"] = "Break-ins everywhere. We need a police station closer than the next city over.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Feels like crime’s the only thing booming in this neighborhood."] = {
		["_default"] = "Feels like crime’s the only thing booming in this neighborhood.",
		["Khariboli"] = "Feels like crime’s the only thing booming in this neighborhood.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Every shop’s installing bars on the windows. Maybe build a precinct instead?"] = {
		["_default"] = "Every shop’s installing bars on the windows. Maybe build a precinct instead?",
		["Khariboli"] = "Every shop’s installing bars on the windows. Maybe build a precinct instead?",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Someone stole my bike again. At this point I should just rent it to them."] = {
		["_default"] = "Someone stole my bike again. At this point I should just rent it to them.",
		["Khariboli"] = "Someone stole my bike again. At this point I should just rent it to them.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Finally, a police precinct opened nearby. Streets already feel calmer."] = {
		["_default"] = "Finally, a police precinct opened nearby. Streets already feel calmer.",
		["Khariboli"] = "Finally, a police precinct opened nearby. Streets already feel calmer.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Just saw officers patrolling downtown. Feels safer than it’s been in years."] = {
		["_default"] = "Just saw officers patrolling downtown. Feels safer than it’s been in years.",
		["Khariboli"] = "Just saw officers patrolling downtown. Feels safer than it’s been in years.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Crime dropped fast once the new Police Dept went up. Great work, city!"] = {
		["_default"] = "Crime dropped fast once the new Police Dept went up. Great work, city!",
		["Khariboli"] = "Crime dropped fast once the new Police Dept went up. Great work, city!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Courthouse finally opened! Justice might actually happen now!"] = {
		["_default"] = "Courthouse finally opened! Justice might actually happen now!",
		["Khariboli"] = "Courthouse finally opened! Justice might actually happen now!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Seeing blue lights used to mean trouble. Now it means peace of mind."] = {
		["_default"] = "Seeing blue lights used to mean trouble. Now it means peace of mind.",
		["Khariboli"] = "Seeing blue lights used to mean trouble. Now it means peace of mind.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Police doing their best, but there’s only so many of them. Fund the department!"] = {
		["_default"] = "Police doing their best, but there’s only so many of them. Fund the department!",
		["Khariboli"] = "Police doing their best, but there’s only so many of them. Fund the department!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Response times are slow. Maybe the precinct needs more vehicles."] = {
		["_default"] = "Response times are slow. Maybe the precinct needs more vehicles.",
		["Khariboli"] = "Response times are slow. Maybe the precinct needs more vehicles.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Wouldn’t mind a few more patrols in the alley behind my store."] = {
		["_default"] = "Wouldn’t mind a few more patrols in the alley behind my store.",
		["Khariboli"] = "Wouldn’t mind a few more patrols in the alley behind my store.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The courthouse backlog is wild! Cases from last year still waiting."] = {
		["_default"] = "The courthouse backlog is wild! Cases from last year still waiting.",
		["Khariboli"] = "The courthouse backlog is wild! Cases from last year still waiting.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["If they had better funding, maybe the cops could stop using scooters."] = {
		["_default"] = "If they had better funding, maybe the cops could stop using scooters.",
		["Khariboli"] = "If they had better funding, maybe the cops could stop using scooters.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The new precinct’s coffee is apparently better than the café’s."] = {
		["_default"] = "The new precinct’s coffee is apparently better than the café’s.",
		["Khariboli"] = "The new precinct’s coffee is apparently better than the café’s.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Finally got my wallet back from lost-and-found. Didn’t expect that level of service!"] = {
		["_default"] = "Finally got my wallet back from lost-and-found. Didn’t expect that level of service!",
		["Khariboli"] = "Finally got my wallet back from lost-and-found. Didn’t expect that level of service!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Saw an officer petting a stray dog on patrol. City’s really turning around."] = {
		["_default"] = "Saw an officer petting a stray dog on patrol. City’s really turning around.",
		["Khariboli"] = "Saw an officer petting a stray dog on patrol. City’s really turning around.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Funny how crime drops when people realize there’s actually a police station now."] = {
		["_default"] = "Funny how crime drops when people realize there’s actually a police station now.",
		["Khariboli"] = "Funny how crime drops when people realize there’s actually a police station now.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["A single spark out here and the whole block’s gone. Where are the firefighters?"] = {
		["_default"] = "A single spark out here and the whole block’s gone. Where are the firefighters?",
		["Khariboli"] = "A single spark out here and the whole block’s gone. Where are the firefighters?",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["My neighbor’s grill caught fire and we had to use buckets. That can’t be normal city life."] = {
		["_default"] = "My neighbor’s grill caught fire and we had to use buckets. That can’t be normal city life.",
		["Khariboli"] = "My neighbor’s grill caught fire and we had to use buckets. That can’t be normal city life.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["No fire station for miles...guess we’re all volunteers now."] = {
		["_default"] = "No fire station for miles...guess we’re all volunteers now.",
		["Khariboli"] = "No fire station for miles...guess we’re all volunteers now.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Every time I smell smoke, I start packing. We need a fire dept."] = {
		["_default"] = "Every time I smell smoke, I start packing. We need a fire dept.",
		["Khariboli"] = "Every time I smell smoke, I start packing. We need a fire dept.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Finally, the new Fire Dept opened nearby. Feels good knowing someone’s watching the flames."] = {
		["_default"] = "Finally, the new Fire Dept opened nearby. Feels good knowing someone’s watching the flames.",
		["Khariboli"] = "Finally, the new Fire Dept opened nearby. Feels good knowing someone’s watching the flames.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The new Fire Station looks incredible. Hope I never need it, but I’m glad it’s there."] = {
		["_default"] = "The new Fire Station looks incredible. Hope I never need it, but I’m glad it’s there.",
		["Khariboli"] = "The new Fire Station looks incredible. Hope I never need it, but I’m glad it’s there.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Heard sirens this morning! Quick response, city’s improving!"] = {
		["_default"] = "Heard sirens this morning! Quick response, city’s improving!",
		["Khariboli"] = "Heard sirens this morning! Quick response, city’s improving!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Firefighters saved the bakery! Free muffins for heroes."] = {
		["_default"] = "Firefighters saved the bakery! Free muffins for heroes.",
		["Khariboli"] = "Firefighters saved the bakery! Free muffins for heroes.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Finally, hydrants with pressure. My water hose isn’t the first line of defense anymore."] = {
		["_default"] = "Finally, hydrants with pressure. My water hose isn’t the first line of defense anymore.",
		["Khariboli"] = "Finally, hydrants with pressure. My water hose isn’t the first line of defense anymore.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Fire Dept’s underfunded. They’re still driving 20-year-old trucks!"] = {
		["_default"] = "Fire Dept’s underfunded. They’re still driving 20-year-old trucks!",
		["Khariboli"] = "Fire Dept’s underfunded. They’re still driving 20-year-old trucks!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["More firefighters, less fireworks! Give them proper funding!"] = {
		["_default"] = "More firefighters, less fireworks! Give them proper funding!",
		["Khariboli"] = "More firefighters, less fireworks! Give them proper funding!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Response times are slow lately. Maybe they need more stations."] = {
		["_default"] = "Response times are slow lately. Maybe they need more stations.",
		["Khariboli"] = "Response times are slow lately. Maybe they need more stations.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["If the Fire Precinct had more funding, maybe insurance wouldn’t be this high."] = {
		["_default"] = "If the Fire Precinct had more funding, maybe insurance wouldn’t be this high.",
		["Khariboli"] = "If the Fire Precinct had more funding, maybe insurance wouldn’t be this high.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Firefighters are doing their best, but the city’s growing faster than their budget."] = {
		["_default"] = "Firefighters are doing their best, but the city’s growing faster than their budget.",
		["Khariboli"] = "Firefighters are doing their best, but the city’s growing faster than their budget.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Someone left their toast on again. I heard the sirens before the smoke alarm."] = {
		["_default"] = "Someone left their toast on again. I heard the sirens before the smoke alarm.",
		["Khariboli"] = "Someone left their toast on again. I heard the sirens before the smoke alarm.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["If the Fire Station had a café, I’d stop by just to say thanks."] = {
		["_default"] = "If the Fire Station had a café, I’d stop by just to say thanks.",
		["Khariboli"] = "If the Fire Station had a café, I’d stop by just to say thanks.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Shoutout to the Fire Dept! They’re faster than my internet."] = {
		["_default"] = "Shoutout to the Fire Dept! They’re faster than my internet.",
		["Khariboli"] = "Shoutout to the Fire Dept! They’re faster than my internet.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Got sick again and there’s still no clinic in this district."] = {
		["_default"] = "Got sick again and there’s still no clinic in this district.",
		["Khariboli"] = "Got sick again and there’s still no clinic in this district.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The nearest hospital is two bus rides away. Hope I survive the trip."] = {
		["_default"] = "The nearest hospital is two bus rides away. Hope I survive the trip.",
		["Khariboli"] = "The nearest hospital is two bus rides away. Hope I survive the trip.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Can’t even get an appointment. We need more doctors, not more paperwork."] = {
		["_default"] = "Can’t even get an appointment. We need more doctors, not more paperwork.",
		["Khariboli"] = "Can’t even get an appointment. We need more doctors, not more paperwork.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Small Clinic is closed again. Guess I’ll just tough it out."] = {
		["_default"] = "Small Clinic is closed again. Guess I’ll just tough it out.",
		["Khariboli"] = "Small Clinic is closed again. Guess I’ll just tough it out.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["No emergency care nearby. If you get hurt here, good luck."] = {
		["_default"] = "No emergency care nearby. If you get hurt here, good luck.",
		["Khariboli"] = "No emergency care nearby. If you get hurt here, good luck.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The new City Hospital looks amazing! Finally, real healthcare in town."] = {
		["_default"] = "The new City Hospital looks amazing! Finally, real healthcare in town.",
		["Khariboli"] = "The new City Hospital looks amazing! Finally, real healthcare in town.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Finally, a Local Hospital nearby. No more traveling halfway across the city for a checkup."] = {
		["_default"] = "Finally, a Local Hospital nearby. No more traveling halfway across the city for a checkup.",
		["Khariboli"] = "Finally, a Local Hospital nearby. No more traveling halfway across the city for a checkup.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Got treated at the Small Clinic today! Fast, clean, and professional. Kudos to the staff!"] = {
		["_default"] = "Got treated at the Small Clinic today! Fast, clean, and professional. Kudos to the staff!",
		["Khariboli"] = "Got treated at the Small Clinic today! Fast, clean, and professional. Kudos to the staff!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Major Hospital opened and it’s already saving lives. Great job, city!"] = {
		["_default"] = "The Major Hospital opened and it’s already saving lives. Great job, city!",
		["Khariboli"] = "The Major Hospital opened and it’s already saving lives. Great job, city!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Feeling safer just knowing there’s a functioning hospital nearby."] = {
		["_default"] = "Feeling safer just knowing there’s a functioning hospital nearby.",
		["Khariboli"] = "Feeling safer just knowing there’s a functioning hospital nearby.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Doctors are overworked and patients keep piling in! Build another hospital!"] = {
		["_default"] = "Doctors are overworked and patients keep piling in! Build another hospital!",
		["Khariboli"] = "Doctors are overworked and patients keep piling in! Build another hospital!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Clinic’s great, but they could use more nurses. Everyone’s exhausted."] = {
		["_default"] = "Clinic’s great, but they could use more nurses. Everyone’s exhausted.",
		["Khariboli"] = "Clinic’s great, but they could use more nurses. Everyone’s exhausted.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The ER waiting room looks like a concert lineup. Fund the health system already!"] = {
		["_default"] = "The ER waiting room looks like a concert lineup. Fund the health system already!",
		["Khariboli"] = "The ER waiting room looks like a concert lineup. Fund the health system already!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Healthcare’s stretched too thin! Patients in hallways again."] = {
		["_default"] = "Healthcare’s stretched too thin! Patients in hallways again.",
		["Khariboli"] = "Healthcare’s stretched too thin! Patients in hallways again.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["If the City Hospital got better funding, maybe we wouldn’t have two-hour ambulance waits."] = {
		["_default"] = "If the City Hospital got better funding, maybe we wouldn’t have two-hour ambulance waits.",
		["Khariboli"] = "If the City Hospital got better funding, maybe we wouldn’t have two-hour ambulance waits.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Big thanks to the clinic nurse who still smiles after twelve-hour shifts."] = {
		["_default"] = "Big thanks to the clinic nurse who still smiles after twelve-hour shifts.",
		["Khariboli"] = "Big thanks to the clinic nurse who still smiles after twelve-hour shifts.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Someone coughed on the bus and ten people panicked. We really need better hospitals."] = {
		["_default"] = "Someone coughed on the bus and ten people panicked. We really need better hospitals.",
		["Khariboli"] = "Someone coughed on the bus and ten people panicked. We really need better hospitals.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["They opened a Major Hospital and the pharmacy lines doubled overnight. Progress?"] = {
		["_default"] = "They opened a Major Hospital and the pharmacy lines doubled overnight. Progress?",
		["Khariboli"] = "They opened a Major Hospital and the pharmacy lines doubled overnight. Progress?",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["My kid said the clinic lollipops taste like medicine. Honestly, same."] = {
		["_default"] = "My kid said the clinic lollipops taste like medicine. Honestly, same.",
		["Khariboli"] = "My kid said the clinic lollipops taste like medicine. Honestly, same.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Doctors and firefighters should get statues before politicians do."] = {
		["_default"] = "Doctors and firefighters should get statues before politicians do.",
		["Khariboli"] = "Doctors and firefighters should get statues before politicians do.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Our kids deserve real classrooms, not overflow in hallways. Build more schools!"] = {
		["_default"] = "Our kids deserve real classrooms, not overflow in hallways. Build more schools!",
		["Khariboli"] = "Our kids deserve real classrooms, not overflow in hallways. Build more schools!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Buses are shipping students across town again. We need a local school now."] = {
		["_default"] = "Buses are shipping students across town again. We need a local school now.",
		["Khariboli"] = "Buses are shipping students across town again. We need a local school now.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Textbooks are older than the dionsaurs in them. Can we fund education like we mean it?"] = {
		["_default"] = "Textbooks are older than the dionsaurs in them. Can we fund education like we mean it?",
		["Khariboli"] = "Textbooks are older than the dionsaurs in them. Can we fund education like we mean it?",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Smaller class sizes would fix half our problems. Build one more school, please."] = {
		["_default"] = "Smaller class sizes would fix half our problems. Build one more school, please.",
		["Khariboli"] = "Smaller class sizes would fix half our problems. Build one more school, please.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Proud day for the city! Our kids finally have places to learn close to home."] = {
		["_default"] = "Proud day for the city! Our kids finally have places to learn close to home.",
		["Khariboli"] = "Proud day for the city! Our kids finally have places to learn close to home.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The new Middle School looks amazing! Our kids won’t have to commute forever anymore."] = {
		["_default"] = "The new Middle School looks amazing! Our kids won’t have to commute forever anymore.",
		["Khariboli"] = "The new Middle School looks amazing! Our kids won’t have to commute forever anymore.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Middle School is packed already. Guess we should’ve built two."] = {
		["_default"] = "Middle School is packed already. Guess we should’ve built two.",
		["Khariboli"] = "Middle School is packed already. Guess we should’ve built two.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Clubs, sports, and science fairs? Middle School is finally a real hub for families."] = {
		["_default"] = "Clubs, sports, and science fairs? Middle School is finally a real hub for families.",
		["Khariboli"] = "Clubs, sports, and science fairs? Middle School is finally a real hub for families.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Heard the Middle School library got new computers. That’s how you build futures."] = {
		["_default"] = "Heard the Middle School library got new computers. That’s how you build futures.",
		["Khariboli"] = "Heard the Middle School library got new computers. That’s how you build futures.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Drop-off lane at Middle School is chaos. Great school! Now fix the traffic plan!"] = {
		["_default"] = "Drop-off lane at Middle School is chaos. Great school! Now fix the traffic plan!",
		["Khariboli"] = "Drop-off lane at Middle School is chaos. Great school! Now fix the traffic plan!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Private School opened its doors and suddenly uniforms are in fashion."] = {
		["_default"] = "Private School opened its doors and suddenly uniforms are in fashion.",
		["Khariboli"] = "Private School opened its doors and suddenly uniforms are in fashion.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Private School scholarships would go a long way! Talent shouldn’t depend on wallets."] = {
		["_default"] = "Private School scholarships would go a long way! Talent shouldn’t depend on wallets.",
		["Khariboli"] = "Private School scholarships would go a long way! Talent shouldn’t depend on wallets.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Heard Private School has small classes and serious teachers. Sounds like results incoming."] = {
		["_default"] = "Heard Private School has small classes and serious teachers. Sounds like results incoming.",
		["Khariboli"] = "Heard Private School has small classes and serious teachers. Sounds like results incoming.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Not my budget, but I’m glad Private School takes pressure off the public system."] = {
		["_default"] = "Not my budget, but I’m glad Private School takes pressure off the public system.",
		["Khariboli"] = "Not my budget, but I’m glad Private School takes pressure off the public system.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Private School debate team is sweeping tournaments. City pride, fancy blazers edition."] = {
		["_default"] = "Private School debate team is sweeping tournaments. City pride, fancy blazers edition.",
		["Khariboli"] = "Private School debate team is sweeping tournaments. City pride, fancy blazers edition.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Museum’s finally open! Weekend plans solved!"] = {
		["_default"] = "Museum’s finally open! Weekend plans solved!",
		["Khariboli"] = "Museum’s finally open! Weekend plans solved!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Field trips to the Museum beat worksheets every time. Thank you, city!"] = {
		["_default"] = "Field trips to the Museum beat worksheets every time. Thank you, city!",
		["Khariboli"] = "Field trips to the Museum beat worksheets every time. Thank you, city!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Rotating exhibits at the Museum keep downtown lively and local businesses busy."] = {
		["_default"] = "Rotating exhibits at the Museum keep downtown lively and local businesses busy.",
		["Khariboli"] = "Rotating exhibits at the Museum keep downtown lively and local businesses busy.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["More late-night hours at the Museum, please some of us work days!"] = {
		["_default"] = "More late-night hours at the Museum, please some of us work days!",
		["Khariboli"] = "More late-night hours at the Museum, please some of us work days!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Museum gift shop is a trap and I’m happily stuck. Education + souvenirs = win."] = {
		["_default"] = "The Museum gift shop is a trap and I’m happily stuck. Education + souvenirs = win.",
		["Khariboli"] = "The Museum gift shop is a trap and I’m happily stuck. Education + souvenirs = win.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Local News Station is live finally! City updates that aren’t rumors."] = {
		["_default"] = "Local News Station is live finally! City updates that aren’t rumors.",
		["Khariboli"] = "Local News Station is live finally! City updates that aren’t rumors.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["News Station covering school board meetings means less drama, more facts. Love it."] = {
		["_default"] = "News Station covering school board meetings means less drama, more facts. Love it.",
		["Khariboli"] = "News Station covering school board meetings means less drama, more facts. Love it.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Traffic and weather from the News Station actually saved me time today. Journalism works!"] = {
		["_default"] = "Traffic and weather from the News Station actually saved me time today. Journalism works!",
		["Khariboli"] = "Traffic and weather from the News Station actually saved me time today. Journalism works!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["If the News Station keeps highlighting achievements, more families will move here. Smart play, city."] = {
		["_default"] = "If the News Station keeps highlighting achievements, more families will move here. Smart play, city.",
		["Khariboli"] = "If the News Station keeps highlighting achievements, more families will move here. Smart play, city.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Shoutout to local reporters asking real questions. Keep schools accountable, keep us informed."] = {
		["_default"] = "Shoutout to local reporters asking real questions. Keep schools accountable, keep us informed.",
		["Khariboli"] = "Shoutout to local reporters asking real questions. Keep schools accountable, keep us informed.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["This city needs more places to unwind before we all burn out."] = {
		["_default"] = "This city needs more places to unwind before we all burn out.",
		["Khariboli"] = "This city needs more places to unwind before we all burn out.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Weekends feel better when there’s somewhere calm to go and clear your head."] = {
		["_default"] = "Weekends feel better when there’s somewhere calm to go and clear your head.",
		["Khariboli"] = "Weekends feel better when there’s somewhere calm to go and clear your head.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Public spaces pay for themselves in community spirit. Build more, argue less."] = {
		["_default"] = "Public spaces pay for themselves in community spirit. Build more, argue less.",
		["Khariboli"] = "Public spaces pay for themselves in community spirit. Build more, argue less.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Hotel just opened downtown! Tourists incoming and local shops smiling already."] = {
		["_default"] = "Hotel just opened downtown! Tourists incoming and local shops smiling already.",
		["Khariboli"] = "Hotel just opened downtown! Tourists incoming and local shops smiling already.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Conference space at the Hotel means real business travel now! Good for the whole city."] = {
		["_default"] = "Conference space at the Hotel means real business travel now! Good for the whole city.",
		["Khariboli"] = "Conference space at the Hotel means real business travel now! Good for the whole city.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["If the Hotel improves transit shuttles, guests won’t flood the streets with taxis. Win-win."] = {
		["_default"] = "If the Hotel improves transit shuttles, guests won’t flood the streets with taxis. Win-win.",
		["Khariboli"] = "If the Hotel improves transit shuttles, guests won’t flood the streets with taxis. Win-win.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Hotel rooftop views are unreal! Instant postcard material for visitors and locals alike."] = {
		["_default"] = "Hotel rooftop views are unreal! Instant postcard material for visitors and locals alike.",
		["Khariboli"] = "Hotel rooftop views are unreal! Instant postcard material for visitors and locals alike.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Nice to see the Hotel hiring locally! Puts money right back into the neighborhood."] = {
		["_default"] = "Nice to see the Hotel hiring locally! Puts money right back into the neighborhood.",
		["Khariboli"] = "Nice to see the Hotel hiring locally! Puts money right back into the neighborhood.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The new movie theater revived date night. Popcorn economy booming!"] = {
		["_default"] = "The new movie theater revived date night. Popcorn economy booming!",
		["Khariboli"] = "The new movie theater revived date night. Popcorn economy booming!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Festival screenings at the theater are drawing crowds! Downtown feels alive again."] = {
		["_default"] = "Festival screenings at the theater are drawing crowds! Downtown feels alive again.",
		["Khariboli"] = "Festival screenings at the theater are drawing crowds! Downtown feels alive again.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Free popcorn would help families! Any chance the theater can make that happen?"] = {
		["_default"] = "Free popcorn would help families! Any chance the theater can make that happen?",
		["Khariboli"] = "Free popcorn would help families! Any chance the theater can make that happen?",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Love the theater, but staggering showtimes could ease the parking crunch by a lot."] = {
		["_default"] = "Love the theater, but staggering showtimes could ease the parking crunch by a lot.",
		["Khariboli"] = "Love the theater, but staggering showtimes could ease the parking crunch by a lot.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Community film nights at the theater? Local shorts before the feature would be awesome."] = {
		["_default"] = "Community film nights at the theater? Local shorts before the feature would be awesome.",
		["Khariboli"] = "Community film nights at the theater? Local shorts before the feature would be awesome.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Church feels like a real community hub: quiet mornings, helpful people, open doors."] = {
		["_default"] = "The Church feels like a real community hub: quiet mornings, helpful people, open doors.",
		["Khariboli"] = "The Church feels like a real community hub: quiet mornings, helpful people, open doors.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Mosque feels like a real community hub: quiet mornings, helpful people, open doors."] = {
		["_default"] = "The Mosque feels like a real community hub: quiet mornings, helpful people, open doors.",
		["Khariboli"] = "The Mosque feels like a real community hub: quiet mornings, helpful people, open doors.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Shinto Temple feels like a real community hub: quiet mornings, helpful people, open doors."] = {
		["_default"] = "The Shinto Temple feels like a real community hub: quiet mornings, helpful people, open doors.",
		["Khariboli"] = "The Shinto Temple feels like a real community hub: quiet mornings, helpful people, open doors.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Hindu Temple feels like a real community hub: quiet mornings, helpful people, open doors."] = {
		["_default"] = "The Hindu Temple feels like a real community hub: quiet mornings, helpful people, open doors.",
		["Khariboli"] = "The Hindu Temple feels like a real community hub: quiet mornings, helpful people, open doors.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Buddha Statue plaza feels like a real community hub: quiet mornings, helpful people, open doors."] = {
		["_default"] = "The Buddha Statue plaza feels like a real community hub: quiet mornings, helpful people, open doors.",
		["Khariboli"] = "The Buddha Statue plaza feels like a real community hub: quiet mornings, helpful people, open doors.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Could the Church extend weekend hours? Lots of families would appreciate evening programs."] = {
		["_default"] = "Could the Church extend weekend hours? Lots of families would appreciate evening programs.",
		["Khariboli"] = "Could the Church extend weekend hours? Lots of families would appreciate evening programs.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Could the Mosque extend weekend hours? Lots of families would appreciate evening programs."] = {
		["_default"] = "Could the Mosque extend weekend hours? Lots of families would appreciate evening programs.",
		["Khariboli"] = "Could the Mosque extend weekend hours? Lots of families would appreciate evening programs.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Could the Shinto Temple extend weekend hours? Lots of families would appreciate evening programs."] = {
		["_default"] = "Could the Shinto Temple extend weekend hours? Lots of families would appreciate evening programs.",
		["Khariboli"] = "Could the Shinto Temple extend weekend hours? Lots of families would appreciate evening programs.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Could the Hindu Temple extend weekend hours? Lots of families would appreciate evening programs."] = {
		["_default"] = "Could the Hindu Temple extend weekend hours? Lots of families would appreciate evening programs.",
		["Khariboli"] = "Could the Hindu Temple extend weekend hours? Lots of families would appreciate evening programs.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Could the Buddha Statue site extend weekend hours? Lots of families would appreciate evening programs."] = {
		["_default"] = "Could the Buddha Statue site extend weekend hours? Lots of families would appreciate evening programs.",
		["Khariboli"] = "Could the Buddha Statue site extend weekend hours? Lots of families would appreciate evening programs.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["An interfaith open house at the Church would be great! Meet neighbors, share food, build trust."] = {
		["_default"] = "An interfaith open house at the Church would be great! Meet neighbors, share food, build trust.",
		["Khariboli"] = "An interfaith open house at the Church would be great! Meet neighbors, share food, build trust.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["An interfaith open house at the Mosque would be great! Meet neighbors, share food, build trust."] = {
		["_default"] = "An interfaith open house at the Mosque would be great! Meet neighbors, share food, build trust.",
		["Khariboli"] = "An interfaith open house at the Mosque would be great! Meet neighbors, share food, build trust.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["An interfaith open house at the Shinto Temple would be great! Meet neighbors, share food, build trust."] = {
		["_default"] = "An interfaith open house at the Shinto Temple would be great! Meet neighbors, share food, build trust.",
		["Khariboli"] = "An interfaith open house at the Shinto Temple would be great! Meet neighbors, share food, build trust.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["An interfaith open house at the Hindu Temple would be great! Meet neighbors, share food, build trust."] = {
		["_default"] = "An interfaith open house at the Hindu Temple would be great! Meet neighbors, share food, build trust.",
		["Khariboli"] = "An interfaith open house at the Hindu Temple would be great! Meet neighbors, share food, build trust.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["An interfaith open house at the Buddha Statue plaza would be great! Meet neighbors, share food, build trust."] = {
		["_default"] = "An interfaith open house at the Buddha Statue plaza would be great! Meet neighbors, share food, build trust.",
		["Khariboli"] = "An interfaith open house at the Buddha Statue plaza would be great! Meet neighbors, share food, build trust.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Big thanks to the Church volunteers turning the hall into a relief center during emergencies. Heroes."] = {
		["_default"] = "Big thanks to the Church volunteers turning the hall into a relief center during emergencies. Heroes.",
		["Khariboli"] = "Big thanks to the Church volunteers turning the hall into a relief center during emergencies. Heroes.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Big thanks to the Mosque volunteers turning the hall into a relief center during emergencies. Heroes."] = {
		["_default"] = "Big thanks to the Mosque volunteers turning the hall into a relief center during emergencies. Heroes.",
		["Khariboli"] = "Big thanks to the Mosque volunteers turning the hall into a relief center during emergencies. Heroes.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Big thanks to the Shinto Temple volunteers turning the hall into a relief center during emergencies. Heroes."] = {
		["_default"] = "Big thanks to the Shinto Temple volunteers turning the hall into a relief center during emergencies. Heroes.",
		["Khariboli"] = "Big thanks to the Shinto Temple volunteers turning the hall into a relief center during emergencies. Heroes.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Big thanks to the Hindu Temple volunteers turning the hall into a relief center during emergencies. Heroes."] = {
		["_default"] = "Big thanks to the Hindu Temple volunteers turning the hall into a relief center during emergencies. Heroes.",
		["Khariboli"] = "Big thanks to the Hindu Temple volunteers turning the hall into a relief center during emergencies. Heroes.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Big thanks to the Buddha Statue caretakers turning the grounds into a relief center during emergencies. Heroes."] = {
		["_default"] = "Big thanks to the Buddha Statue caretakers turning the grounds into a relief center during emergencies. Heroes.",
		["Khariboli"] = "Big thanks to the Buddha Statue caretakers turning the grounds into a relief center during emergencies. Heroes.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Still no Bus Depot in this district; half the routes end in the middle of nowhere."] = {
		["_default"] = "Still no Bus Depot in this district; half the routes end in the middle of nowhere.",
		["Khariboli"] = "Still no Bus Depot in this district; half the routes end in the middle of nowhere.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["No depot means buses break down on the street. Maybe build one before the fleet collapses?"] = {
		["_default"] = "No depot means buses break down on the street. Maybe build one before the fleet collapses?",
		["Khariboli"] = "No depot means buses break down on the street. Maybe build one before the fleet collapses?",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Waiting for a bus that never comes. This neighborhood needs proper service."] = {
		["_default"] = "Waiting for a bus that never comes. This neighborhood needs proper service.",
		["Khariboli"] = "Waiting for a bus that never comes. This neighborhood needs proper service.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The new Bus Depot changed everything. Buses actually arrive on time now!"] = {
		["_default"] = "The new Bus Depot changed everything. Buses actually arrive on time now!",
		["Khariboli"] = "The new Bus Depot changed everything. Buses actually arrive on time now!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Depot is open, routes are smooth, and commuting finally feels civilized."] = {
		["_default"] = "Depot is open, routes are smooth, and commuting finally feels civilized.",
		["Khariboli"] = "Depot is open, routes are smooth, and commuting finally feels civilized.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Seeing freshly cleaned buses every morning gives me more faith in the city than most speeches do."] = {
		["_default"] = "Seeing freshly cleaned buses every morning gives me more faith in the city than most speeches do.",
		["Khariboli"] = "Seeing freshly cleaned buses every morning gives me more faith in the city than most speeches do.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Bus Depot is working overtime. Time to expand before rush hour destroys it."] = {
		["_default"] = "Bus Depot is working overtime. Time to expand before rush hour destroys it.",
		["Khariboli"] = "Bus Depot is working overtime. Time to expand before rush hour destroys it.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["We need more electric buses. The depot could lead the way in going green."] = {
		["_default"] = "We need more electric buses. The depot could lead the way in going green.",
		["Khariboli"] = "We need more electric buses. The depot could lead the way in going green.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Depot is too small for a growing city. Fund upgrades before delays return."] = {
		["_default"] = "Depot is too small for a growing city. Fund upgrades before delays return.",
		["Khariboli"] = "Depot is too small for a growing city. Fund upgrades before delays return.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Drivers deserve better facilities too. Fund the depot break rooms and workshops."] = {
		["_default"] = "Drivers deserve better facilities too. Fund the depot break rooms and workshops.",
		["Khariboli"] = "Drivers deserve better facilities too. Fund the depot break rooms and workshops.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["No Airport means we are cut off from the world. Businesses and tourism are stuck on the ground."] = {
		["_default"] = "No Airport means we are cut off from the world. Businesses and tourism are stuck on the ground.",
		["Khariboli"] = "No Airport means we are cut off from the world. Businesses and tourism are stuck on the ground.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["We have skyscrapers but no Airport. How do people even visit this place?"] = {
		["_default"] = "We have skyscrapers but no Airport. How do people even visit this place?",
		["Khariboli"] = "We have skyscrapers but no Airport. How do people even visit this place?",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Driving to another city just to fly is ridiculous. Build an Airport already!"] = {
		["_default"] = "Driving to another city just to fly is ridiculous. Build an Airport already!",
		["Khariboli"] = "Driving to another city just to fly is ridiculous. Build an Airport already!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Finally, flights are running and the Airport looks incredible."] = {
		["_default"] = "Finally, flights are running and the Airport looks incredible.",
		["Khariboli"] = "Finally, flights are running and the Airport looks incredible.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Airport security is smooth, shops are open, and it actually feels world class."] = {
		["_default"] = "Airport security is smooth, shops are open, and it actually feels world class.",
		["Khariboli"] = "Airport security is smooth, shops are open, and it actually feels world class.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Seeing planes overhead again feels like the city is truly connected to the world."] = {
		["_default"] = "Seeing planes overhead again feels like the city is truly connected to the world.",
		["Khariboli"] = "Seeing planes overhead again feels like the city is truly connected to the world.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Airport has boosted local hotels and restaurants overnight. Smart investment!"] = {
		["_default"] = "The Airport has boosted local hotels and restaurants overnight. Smart investment!",
		["Khariboli"] = "The Airport has boosted local hotels and restaurants overnight. Smart investment!",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Airport is great, but we need a second terminal before travelers start camping on the floor."] = {
		["_default"] = "Airport is great, but we need a second terminal before travelers start camping on the floor.",
		["Khariboli"] = "Airport is great, but we need a second terminal before travelers start camping on the floor.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Customs lines are brutal. Time for more staff and faster systems."] = {
		["_default"] = "Customs lines are brutal. Time for more staff and faster systems.",
		["Khariboli"] = "Customs lines are brutal. Time for more staff and faster systems.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Airport expansion would mean new routes, new jobs, and fewer layovers. Let's do it."] = {
		["_default"] = "Airport expansion would mean new routes, new jobs, and fewer layovers. Let's do it.",
		["Khariboli"] = "Airport expansion would mean new routes, new jobs, and fewer layovers. Let's do it.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The cargo side of the Airport could use more funding too. Exports keep the city running."] = {
		["_default"] = "The cargo side of the Airport could use more funding too. Exports keep the city running.",
		["Khariboli"] = "The cargo side of the Airport could use more funding too. Exports keep the city running.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["We need a Metro system already. The buses can only do so much."] = {
		["_default"] = "We need a Metro system already. The buses can only do so much.",
		["Khariboli"] = "We need a Metro system already. The buses can only do so much.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Traffic is a nightmare. Please, just build a Metro before I lose my mind."] = {
		["_default"] = "Traffic is a nightmare. Please, just build a Metro before I lose my mind.",
		["Khariboli"] = "Traffic is a nightmare. Please, just build a Metro before I lose my mind.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["A Metro would connect the city like nothing else. No more two-hour commutes."] = {
		["_default"] = "A Metro would connect the city like nothing else. No more two-hour commutes.",
		["Khariboli"] = "A Metro would connect the city like nothing else. No more two-hour commutes.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Metro opened this week and it already feels like the city leveled up."] = {
		["_default"] = "The Metro opened this week and it already feels like the city leveled up.",
		["Khariboli"] = "The Metro opened this week and it already feels like the city leveled up.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Fast, clean, quiet. I cannot believe I am saying this about public transport."] = {
		["_default"] = "Fast, clean, quiet. I cannot believe I am saying this about public transport.",
		["Khariboli"] = "Fast, clean, quiet. I cannot believe I am saying this about public transport.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Took the Metro today and got to work early for the first time in years."] = {
		["_default"] = "Took the Metro today and got to work early for the first time in years.",
		["Khariboli"] = "Took the Metro today and got to work early for the first time in years.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Finally, a Metro that makes us feel like a real city. Worth every tax dollar."] = {
		["_default"] = "Finally, a Metro that makes us feel like a real city. Worth every tax dollar.",
		["Khariboli"] = "Finally, a Metro that makes us feel like a real city. Worth every tax dollar.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Rode the Metro at night and swear I saw someone staring back from an empty tunnel."] = {
		["_default"] = "Rode the Metro at night and swear I saw someone staring back from an empty tunnel.",
		["Khariboli"] = "Rode the Metro at night and swear I saw someone staring back from an empty tunnel.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Who keeps talking about Marvin in the tunnels? Is that supposed to be a mascot or a ghost?"] = {
		["_default"] = "Who keeps talking about Marvin in the tunnels? Is that supposed to be a mascot or a ghost?",
		["Khariboli"] = "Who keeps talking about Marvin in the tunnels? Is that supposed to be a mascot or a ghost?",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["People say the Metro hums even when the power is off. I am not checking to find out."] = {
		["_default"] = "People say the Metro hums even when the power is off. I am not checking to find out.",
		["Khariboli"] = "People say the Metro hums even when the power is off. I am not checking to find out.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Metro maintenance crews keep finding toys down there. No one knows where they come from."] = {
		["_default"] = "Metro maintenance crews keep finding toys down there. No one knows where they come from.",
		["Khariboli"] = "Metro maintenance crews keep finding toys down there. No one knows where they come from.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Urban legend says Marvin used to live down there before the Metro opened. Now he just waits."] = {
		["_default"] = "Urban legend says Marvin used to live down there before the Metro opened. Now he just waits.",
		["Khariboli"] = "Urban legend says Marvin used to live down there before the Metro opened. Now he just waits.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Heard the Metro has a hidden station that is not on any map. If you see it, do not get off."] = {
		["_default"] = "Heard the Metro has a hidden station that is not on any map. If you see it, do not get off.",
		["Khariboli"] = "Heard the Metro has a hidden station that is not on any map. If you see it, do not get off.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Metro construction is noisy but worth it. The city finally feels alive again."] = {
		["_default"] = "Metro construction is noisy but worth it. The city finally feels alive again.",
		["Khariboli"] = "Metro construction is noisy but worth it. The city finally feels alive again.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["They say the Metro went over budget again. Still better than sitting in traffic for half my life."] = {
		["_default"] = "They say the Metro went over budget again. Still better than sitting in traffic for half my life.",
		["Khariboli"] = "They say the Metro went over budget again. Still better than sitting in traffic for half my life.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Metro delays again, but at least it is progress. Better late than never."] = {
		["_default"] = "Metro delays again, but at least it is progress. Better late than never.",
		["Khariboli"] = "Metro delays again, but at least it is progress. Better late than never.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Would love an Archery Range out here. Beats staring at empty lots all weekend."] = {
		["_default"] = "Would love an Archery Range out here. Beats staring at empty lots all weekend.",
		["Khariboli"] = "Would love an Archery Range out here. Beats staring at empty lots all weekend.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Archery Range is finally open. Calm, quiet, and precise. Best stress relief in the city."] = {
		["_default"] = "Archery Range is finally open. Calm, quiet, and precise. Best stress relief in the city.",
		["Khariboli"] = "Archery Range is finally open. Calm, quiet, and precise. Best stress relief in the city.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Archery Range is way better than another mall. Focus over shopping any day."] = {
		["_default"] = "Archery Range is way better than another mall. Focus over shopping any day.",
		["Khariboli"] = "Archery Range is way better than another mall. Focus over shopping any day.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Basketball Court is packed every afternoon. Easily the best community spot around."] = {
		["_default"] = "The Basketball Court is packed every afternoon. Easily the best community spot around.",
		["Khariboli"] = "The Basketball Court is packed every afternoon. Easily the best community spot around.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Wish we had a Basketball Court nearby. It would keep the kids busy and happy."] = {
		["_default"] = "Wish we had a Basketball Court nearby. It would keep the kids busy and happy.",
		["Khariboli"] = "Wish we had a Basketball Court nearby. It would keep the kids busy and happy.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Basketball Court beats any gym membership. Free, fun, and friendly."] = {
		["_default"] = "Basketball Court beats any gym membership. Free, fun, and friendly.",
		["Khariboli"] = "Basketball Court beats any gym membership. Free, fun, and friendly.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Basketball Stadium days are the loudest, happiest days this city gets."] = {
		["_default"] = "Basketball Stadium days are the loudest, happiest days this city gets.",
		["Khariboli"] = "Basketball Stadium days are the loudest, happiest days this city gets.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["We need a real Basketball Stadium so the team stops borrowing arenas."] = {
		["_default"] = "We need a real Basketball Stadium so the team stops borrowing arenas.",
		["Khariboli"] = "We need a real Basketball Stadium so the team stops borrowing arenas.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The new Basketball Stadium is incredible. Feels like a major city now."] = {
		["_default"] = "The new Basketball Stadium is incredible. Feels like a major city now.",
		["Khariboli"] = "The new Basketball Stadium is incredible. Feels like a major city now.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Football Stadium crowds are massive. Local economy is loving game days."] = {
		["_default"] = "Football Stadium crowds are massive. Local economy is loving game days.",
		["Khariboli"] = "Football Stadium crowds are massive. Local economy is loving game days.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Still waiting on a Football Stadium here. Everyone keeps driving to the next city."] = {
		["_default"] = "Still waiting on a Football Stadium here. Everyone keeps driving to the next city.",
		["Khariboli"] = "Still waiting on a Football Stadium here. Everyone keeps driving to the next city.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The new Football Stadium puts our city on the map."] = {
		["_default"] = "The new Football Stadium puts our city on the map.",
		["Khariboli"] = "The new Football Stadium puts our city on the map.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Golf Course opened this week. Finally a reason for business meetings outdoors."] = {
		["_default"] = "Golf Course opened this week. Finally a reason for business meetings outdoors.",
		["Khariboli"] = "Golf Course opened this week. Finally a reason for business meetings outdoors.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["A Golf Course would look great here. Better than another office park."] = {
		["_default"] = "A Golf Course would look great here. Better than another office park.",
		["Khariboli"] = "A Golf Course would look great here. Better than another office park.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Golf Course greens make this part of town look alive. Best landscaping in the city."] = {
		["_default"] = "Golf Course greens make this part of town look alive. Best landscaping in the city.",
		["Khariboli"] = "Golf Course greens make this part of town look alive. Best landscaping in the city.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Public Pool is open and the whole neighborhood showed up. Best summer in years."] = {
		["_default"] = "Public Pool is open and the whole neighborhood showed up. Best summer in years.",
		["Khariboli"] = "Public Pool is open and the whole neighborhood showed up. Best summer in years.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["We could use a Public Pool. The kids are melting out here."] = {
		["_default"] = "We could use a Public Pool. The kids are melting out here.",
		["Khariboli"] = "We could use a Public Pool. The kids are melting out here.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Public Pool beats every private gym. Cheap, clean, and fun."] = {
		["_default"] = "The Public Pool beats every private gym. Cheap, clean, and fun.",
		["Khariboli"] = "The Public Pool beats every private gym. Cheap, clean, and fun.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Skate Park finally opened. Now we can stop getting yelled at downtown."] = {
		["_default"] = "The Skate Park finally opened. Now we can stop getting yelled at downtown.",
		["Khariboli"] = "The Skate Park finally opened. Now we can stop getting yelled at downtown.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Still no Skate Park here. Guess the stairs will have to do."] = {
		["_default"] = "Still no Skate Park here. Guess the stairs will have to do.",
		["Khariboli"] = "Still no Skate Park here. Guess the stairs will have to do.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Skate Park brings life to this area. Better than another parking lot any day."] = {
		["_default"] = "Skate Park brings life to this area. Better than another parking lot any day.",
		["Khariboli"] = "Skate Park brings life to this area. Better than another parking lot any day.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Soccer Stadium is packed and electric. Nothing beats game day energy."] = {
		["_default"] = "Soccer Stadium is packed and electric. Nothing beats game day energy.",
		["Khariboli"] = "Soccer Stadium is packed and electric. Nothing beats game day energy.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Still no Soccer Stadium. Players keep practicing on empty fields."] = {
		["_default"] = "Still no Soccer Stadium. Players keep practicing on empty fields.",
		["Khariboli"] = "Still no Soccer Stadium. Players keep practicing on empty fields.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Our Soccer Stadium puts the whole city in a good mood."] = {
		["_default"] = "Our Soccer Stadium puts the whole city in a good mood.",
		["Khariboli"] = "Our Soccer Stadium puts the whole city in a good mood.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Tennis Court is spotless and full every morning. Great addition to the neighborhood."] = {
		["_default"] = "The Tennis Court is spotless and full every morning. Great addition to the neighborhood.",
		["Khariboli"] = "The Tennis Court is spotless and full every morning. Great addition to the neighborhood.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Still no Tennis Court nearby. Guess we will keep using the parking lot lines."] = {
		["_default"] = "Still no Tennis Court nearby. Guess we will keep using the parking lot lines.",
		["Khariboli"] = "Still no Tennis Court nearby. Guess we will keep using the parking lot lines.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["The Tennis Court looks great next to the park. Makes the area feel upscale."] = {
		["_default"] = "The Tennis Court looks great next to the park. Makes the area feel upscale.",
		["Khariboli"] = "The Tennis Court looks great next to the park. Makes the area feel upscale.",
		["Punjabi"] = "...",
		["Bhojpuri"] = "...",
		["Haryanvi"] = "...",
	},

	["Was there always a road there? I like it."] = {
		["_default"] = "Was there always a road there? I like it.",
		["Khariboli"] = "क्या वहाँ हमेशा से सड़क थी? मुझे यह पसंद है।",
		["Punjabi"] = "ਕੀ ਉਥੇ ਹਮੇਸ਼ਾ ਰਸਤਾ ਸੀ? ਮੈਨੂੰ ਇਹ ਚੰਗਾ ਲੱਗਦਾ ਹੈ।",
		["Bhojpuri"] = "ओहिजा हमेशा से सड़क रहे का? हमके ई पसंद बा।",
		["Haryanvi"] = "क्या उहां हमेशा से सड़क थी? मुझे बढ़िया लागै सै।",
	},

	["Did you know there are more planes in the ocean than submarines in the sky?"] = {
		["_default"] = "Did you know there are more planes in the ocean than submarines in the sky?",
		["Khariboli"] = "क्या आप जानते हैं कि समुद्र में विमानों की संख्या आकाश में पनडुब्बियों से अधिक है?",
		["Punjabi"] = "ਕੀ ਤੁਸੀਂ ਜਾਣਦੇ ਹੋ ਕਿ ਸਮੁੰਦਰ ਵਿੱਚ ਪਲੇਨਾਂ ਦੀ ਗਿਣਤੀ ਆਕਾਸ਼ ਵਿੱਚ ਸਬਮਰੀਨਜ਼ ਨਾਲੋਂ ਵੱਧ ਹੈ?",
		["Bhojpuri"] = "जानत बाड़ऽ की समुंदर में जहाज जादा बा जवन आकास में पनडुब्बी से?",
		["Haryanvi"] = "तने जाण्या सै कि समुंदर में जहाज जादा सै जितनी हवा में पनडुब्बी?",
	},

	["I like the new coffee shop downtown."] = {
		["_default"] = "I like the new coffee shop downtown.",
		["Khariboli"] = "मुझे डाउनटाउन का नया कॉफी शॉप पसंद है।",
		["Punjabi"] = "ਮੈਨੂੰ ਸ਼ਹਿਰ ਦੇ ਨਵੇਂ ਕੌਫੀ ਸ਼ਾਪ ਚੰਗਾ ਲੱਗਦਾ ਹੈ।",
		["Bhojpuri"] = "हमके शहर के नया कॉफी दुकान पसंद बा।",
		["Haryanvi"] = "मने नया कॉफी शॉप पसंद सै जो शहर में खुला है।",
	},

	["Everyone has the right to be stupid, it's just some people abuse the privilege."] = {
		["_default"] = "Everyone has the right to be stupid, it's just some people abuse the privilege.",
		["Khariboli"] = "हर किसी को मूर्ख होने का अधिकार है, बस कुछ लोग इस विशेषाधिकार का दुरुपयोग करते हैं।",
		["Punjabi"] = "ਹਰ ਕਿਸੇ ਨੂੰ ਮੂਰਖ ਹੋਣ ਦਾ ਹੱਕ ਹੈ, ਪਰ ਕੁਝ ਲੋਕ ਇਸ ਹੱਕ ਦਾ ਦੁਰੁਪਯੋਗ ਕਰਦੇ ਹਨ।",
		["Bhojpuri"] = "हर आदमी के बेवकूफ बने के अधिकार बा, बस कुछ लोग एह अधिकार के दुरुपयोग करेलन।",
		["Haryanvi"] = "हर किसी के बेवकूफ होण का अधिकार सै, बस कुछ लोग इस अधिकार का गलत इस्तेमाल करते सै।",
	},

	["My friend said that the old factory has a really cool abandoned tunnel in it. I don't think I'll find out for myself."] = {
		["_default"] = "My friend said that the old factory has a really cool abandoned tunnel in it. I don't think I'll find out for myself.",
		["Khariboli"] = "मेरे दोस्त ने कहा कि पुरानी फैक्ट्री में एक बहुत ही शानदार छोड़ा हुआ सुरंग है। मुझे नहीं लगता कि मैं खुद पता लगाऊंगा।",
		["Punjabi"] = "ਮੇਰੇ ਦੋਸਤ ਨੇ ਕਿਹਾ ਕਿ ਪੁਰਾਣੇ ਫੈਕਟਰੀ ਵਿਚ ਇੱਕ ਬਹੁਤ ਕੁਲ ਛੱਡੀ ਗਈ ਸੁਰੰਗ ਹੈ। ਮੈਂ ਨਹੀਂ ਸੋਚਦਾ ਕਿ ਮੈਂ ਖੁਦ ਪਤਾ ਲਗਾਵਾਂਗਾ।",
		["Bhojpuri"] = "हमार दोस्त कहले कि पुरान फैक्ट्री में एकदम कूल छोड़ाइल सुरंग बा। हम ना लागेला कि हम खुद देखब।",
		["Haryanvi"] = "मेरे दोस्त ने बताया पुरानी फैक्टरी में एक बढ़िया खाली सुरंग सै। मैं खुद पता करने नहीं जाऊंगा।",
	},

	["I dont think inside the box or outside the box... I dont even know where the box is..."] = {
		["_default"] = "I dont think inside the box or outside the box... I dont even know where the box is...",
		["Khariboli"] = "मैं बक्से के अंदर या बाहर सोचता ही नहीं... मुझे तो यह भी नहीं पता कि बक्सा कहाँ है...",
		["Punjabi"] = "ਮੈਂ ਡੱਬੇ ਦੇ ਅੰਦਰ ਜਾਂ ਬਾਹਰ ਨਹੀਂ ਸੋਚਦਾ... ਮੈਨੂੰ ਤਾਂ ਇਹ ਵੀ ਨਹੀਂ ਪਤਾ ਕਿ ਡੱਬਾ ਕਿੱਥੇ ਹੈ...",
		["Bhojpuri"] = "हम डब्बा के अंदर-बाहर सोचबे ना करी... हम त जानत ही ना कि डिब्बा कहाँ बा...",
		["Haryanvi"] = "मैं डिब्बे के अंदर या बाहर नहीं सोचता... मैंनै तो यह भी ना पता कि डिब्बा कहाँ है...",
	},

	["I think I might paint my house blue. I like blue."] = {
		["_default"] = "I think I might paint my house blue. I like blue.",
		["Khariboli"] = "मैं सोच रहा हूँ कि मैं अपना घर नीला रंग दूँ। मुझे नीला पसंद है।",
		["Punjabi"] = "ਮੈਂ ਸੋਚ ਰਿਹਾ ਹਾਂ ਕਿ ਮੈਂ ਆਪਣਾ ਘਰ ਨੀਲਾ ਰੰਗਾਂ। ਮੈਨੂੰ ਨੀਲਾ ਰੰਗ ਪਸੰਦ ਹੈ।",
		["Bhojpuri"] = "हम सोचतानी कि हम अपना घर नीला रंग से रंग दिहीं। हमके नीला पसंद बा।",
		["Haryanvi"] = "मैं सोचूं हूँ अपना घर नीला रंग दूं। नीला रंग भोत पसंद सै।",
	},

	["Things just aren’t what they used to be. And probably never were."] = {
		["_default"] = "Things just aren’t what they used to be. And probably never were.",
		["Khariboli"] = "चीजें वैसी नहीं हैं जैसी वे थीं। और शायद कभी थीं ही नहीं।",
		["Punjabi"] = "ਹਰ ਚੀਜ਼ ਹੁਣ ਉਹ ਨਹੀਂ ਰਹੀ ਜੇਹੀ ਸੀ। ਅਤੇ ਸ਼ਾਇਦ ਕਦੇ ਸੀ ਹੀ ਨਹੀਂ।",
		["Bhojpuri"] = "चीजें अब वैसी ना रह गईल बाड़ीं। आ शायद कबो रहीं ही ना।",
		["Haryanvi"] = "चीजें अब वैसी ना रहीं जैसी पहले थीं। शायद कभी थीं भी ना।",
	},

	["I’ve always wanted to be somebody, but I see now I should’ve been more specific."] = {
		["_default"] = "I’ve always wanted to be somebody, but I see now I should’ve been more specific.",
		["Khariboli"] = "मैं हमेशा किसी बनने की चाह रखता था, लेकिन अब समझ आया कि मुझे अधिक विशिष्ट होना चाहिए था।",
		["Punjabi"] = "ਮੈਂ ਹਮੇਸ਼ਾਂ ਕੋਈ ਵਿਅਕਤੀ ਬਣਨਾ ਚਾਹੁੰਦਾ ਸੀ, ਪਰ ਹੁਣ ਦਿਸਦਾ ਹੈ ਕਿ ਮੈਨੂੰ ਹੋਰ ਖਾਸ ਹੋਣਾ ਚਾਹੀਦਾ ਸੀ।",
		["Bhojpuri"] = "हम हमेशा सोचे रहीं कि कुछ बनब, बाकिर अब बुझाइल कि कुछ खास बनल जरूरी रहे।",
		["Haryanvi"] = "मैं हमेशा कोई बनने की सोचता था, पर अब दिखे है कि मुझे ज़्यादा विशेष होना चाहिए था।",
	},

	["The early bird can have the worm, because worms are gross and mornings are stupid."] = {
		["_default"] = "The early bird can have the worm, because worms are gross and mornings are stupid.",
		["Khariboli"] = "जल्दी उठने वाला पक्षी कीड़ा ले सकता है, क्योंकि कीड़े घिनौने होते हैं और सुबहें बेवकूफ हैं।",
		["Punjabi"] = "ਜਲਦੀ ਉਠਣ ਵਾਲਾ ਪੰਛੀ ਕੀੜਾ ਲੈ ਸਕਦਾ ਹੈ, ਕਿਉਂਕਿ ਕੀੜੇ ਗੰਦੇ ਹੁੰਦੇ ਹਨ ਅਤੇ ਸਵੇਰੇ ਮੂਰਖ ਹੁੰਦੇ ਹਨ।",
		["Bhojpuri"] = "सबसे पहिले उठे वाला चिरई कीड़ा ले सकेला, काहे कि कीड़ा गंदा होला आ भोरहा बेकाम होला।",
		["Haryanvi"] = "जल्दी उठने वाला पंछी कीड़ा खा ले, कीड़े घिनौने होते हैं और सुबह बेवकूफ होती है।",
	},

	["I love the new trees they planted in my neighborhood."] = {
		["_default"] = "I love the new trees they planted in my neighborhood.",
		["Khariboli"] = "मुझे मेरे पड़ोस में लगाए नए पेड़ बहुत पसंद हैं।",
		["Punjabi"] = "ਮੈਨੂੰ ਆਪਣੇ ਪੜੋਸ ਵਿੱਚ ਨਵੇਂ ਲਾਏ ਗਏ ਦਰੱਖ਼ਤਾਂ ਨਾਲ ਪਿਆਰ ਹੈ।",
		["Bhojpuri"] = "हमके अपने मोहल्ला में लगावल नया पेड़ बहुत पसंद बा।",
		["Haryanvi"] = "मने अपने पड़ोस में लगे नए पेड़ बहुत पसंद सै।",
	},

	["Be yourself. No one can ever tell you’re doing it wrong."] = {
		["_default"] = "Be yourself. No one can ever tell you’re doing it wrong.",
		["Khariboli"] = "खुद बनो। कोई भी आपको नहीं बता सकता कि आप यह गलत कर रहे हैं।",
		["Punjabi"] = "ਖੁਦ ਬਣੋ। ਕੋਈ ਵੀ ਨਹੀਂ ਦੱਸ ਸਕਦਾ ਕਿ ਤੁਸੀਂ ਇਹ ਗਲਤ ਕਰ ਰਹੇ ਹੋ।",
		["Bhojpuri"] = "आप खुद रहो। केहू ना बता सकेला कि आप गलत कर रहल बानी।",
		["Haryanvi"] = "खुद रहो। कोई तने ना बता सके कि तू गलत कर रहा है।",
	},

	["My favorite childhood memory, is not paying bills."] = {
		["_default"] = "My favorite childhood memory, is not paying bills.",
		["Khariboli"] = "बचपन की मेरी पसंदीदा याद है बिल न चुकाना।",
		["Punjabi"] = "ਮੇਰੀ ਬਚਪਨ ਦੀ ਮਨਪਸੰਦ ਯਾਦ ਬਿਲ ਨਾ ਭਰਨੀ ਸੀ।",
		["Bhojpuri"] = "हमार बचपन के सबसे बढ़िया याद बा—बिल ना भरल।",
		["Haryanvi"] = "मेरा बचपन का सबसे अच्छा याद: बिल न भरना।",
	},

	["Did you know that birds control time? They do this out of spite."] = {
		["_default"] = "Did you know that birds control time? They do this out of spite.",
		["Khariboli"] = "क्या तुम जानते हो कि पक्षी समय को नियंत्रित करते हैं? वे यह बदले की भावना से करते हैं।",
		["Punjabi"] = "ਕੀ ਤੁਸੀਂ ਜਾਣਦੇ ਹੋ ਕਿ ਪੰਛੀ ਸਮੇਂ ਨੂੰ ਕੰਟਰੋਲ ਕਰਦੇ ਹਨ? ਉਹ ਇਹ ਨਫ਼ਰਤ ਨਾਲ ਕਰਦੇ ਹਨ।",
		["Bhojpuri"] = "तू जानत बाड़ऽ कि चिरई समय के कंट्रोल करेले? ऊ लोग ई जिद्दीपन से करेला।",
		["Haryanvi"] = "तने पता सै कि पक्षी टाइम कंट्रोल करते हैं? वो ये ज़िद में करते हैं।",
	},

	["If someone makes you happy, make them happier."] = {
		["_default"] = "If someone makes you happy, make them happier.",
		["Khariboli"] = "अगर कोई आपको खुश करता है, तो उन्हें और खुश करें।",
		["Punjabi"] = "ਜੇ ਕੋਈ ਤੁਹਾਨੂੰ ਖੁਸ਼ ਕਰਦਾ ਹੈ, ਤਾਂ ਉਨ੍ਹਾਂ ਨੂੰ ਹੋਰ ਖੁਸ਼ ਕਰੋ।",
		["Bhojpuri"] = "अगर केहू आपको खुश करेला, त उन्हें आउर खुश करा।",
		["Haryanvi"] = "अगर कोई तने खुश करे, तो उसे और ज्यादा खुश कर।",
	},

	["I like bananas because they have no bones."] = {
		["_default"] = "I like bananas because they have no bones.",
		["Khariboli"] = "मुझे केले पसंद हैं क्योंकि उनमें हड्डियाँ नहीं होतीं।",
		["Punjabi"] = "ਮੈਨੂੰ ਕੇਲਾ ਪਸੰਦ ਹੈ ਕਿਉਂਕਿ ਉਸ ਵਿੱਚ ਹੱਡੀਆਂ ਨਹੀਂ ਹੁੰਦੀਆਂ।",
		["Bhojpuri"] = "मलाई केला पसन्द बा काहे कि उसमें हड्डी नइखे।",
		["Haryanvi"] = "मने केले पसंद सै क्योंकि उनमें हड्डी ना होवे।",
	},

	["A raccoon ate through my trashcan yesterday. It was a really cute raccoon, so I'm not really mad about it."] = {
		["_default"] = "A raccoon ate through my trashcan yesterday. It was a really cute raccoon, so I'm not really mad about it.",
		["Khariboli"] = "कल एक रैकून ने मेरे कूड़ेदान को कुतर दिया। वह बहुत प्यारा रैकून था, इसलिए मैं वास्तव में नाराज़ नहीं हूँ।",
		["Punjabi"] = "ਕੱਲ੍ਹ ਇੱਕ ਰੈਕੂਨ ਨੇ ਮੇਰੀ ਕੂੜੇਦਾਨ ਨੂੰ ਚੀਰ ਦਿੱਤਾ। ਉਹ ਬਹੁਤ ਪਿਆਰਾ ਰੈਕੂਨ ਸੀ, ਇਸ ਲਈ ਮੈਂ ਗੁੱਸੇ ਨਹੀਂ ਹਾਂ।",
		["Bhojpuri"] = "काल्ह एक रैकून हमरा कूड़ेदान के काट देहिस। ऊ बहुत प्यारा रैकून रहे, त हम नाराज़ नइखी।",
		["Haryanvi"] = "काल एक रैकून नै मेरे कूड़ेदान को कुतर दिया। वो बड़ा प्यारा था, इसलिये मैं नाराज ना हूँ।",
	},

	["Forever is a long time. But not as long as it was yesterday."] = {
		["_default"] = "Forever is a long time. But not as long as it was yesterday.",
		["Khariboli"] = "हमेशा लंबा समय होता है, लेकिन कल जितना लंबा नहीं।",
		["Punjabi"] = "ਸਦਾ ਇੱਕ ਲੰਮਾ ਸਮਾਂ ਹੁੰਦਾ ਹੈ, ਪਰ ਕੱਲ੍ਹ ਤੋਂ ਲੰਮਾ ਨਹੀਂ।",
		["Bhojpuri"] = "सदा बहुते लमहर होला, बाकि कल जेतना ना।",
		["Haryanvi"] = "हमेशा बहुत लंबा समय होता है, पर कल जितना लंबा नहीं।",
	},

	["I've heard they're getting rid of Ohio. It's for the best, really."] = {
		["_default"] = "...",
		["Khariboli"] = "मैंने सुना है कि वे ओहायो को खत्म कर रहे हैं। यह सच में बेहतर के लिए है।",
		["Punjabi"] = "ਮੈਂ ਸੁਣਿਆ ਹੈ ਉਹ ਓਹਾਇਓ ਨੂੰ ਖਤਮ ਕਰ ਰਹੇ ਹਨ। ਇਹ ਸੱਚਮੁੱਚ ਚੰਗੇ ਲਈ ਹੈ।",
		["Bhojpuri"] = "सुनल बानी कि ऊ लोग ओहायो के हटा रहल बा। ई सही में बढ़िया बात ह।",
		["Haryanvi"] = "मैंने सुना सै कि वो लोग ओहायो को खत्म कर रहे हैं। ये सच में अच्छा सै।",
	},

	["Have you seen Marvin? He owes me money."] = {
		["_default"] = "Have you seen Marvin? He owes me money.",
		["Khariboli"] = "क्या तुमने मार्विन को देखा? उसने मुझे पैसे देने हैं।",
		["Punjabi"] = "ਕੀ ਤੁਸੀਂ ਮਾਰਵਿਨ ਨੂੰ ਵੇਖਿਆ ਹੈ? ਉਸ ਨੇ ਮੈਨੂੰ ਪੈਸੇ ਦੇਣੇ ਹਨ।",
		["Bhojpuri"] = "मार्विन के देखल बाड़ऽ? ऊ हमरा पैसा देवे के बा।",
		["Haryanvi"] = "तने मार्विन नै देखा है? वो मुझ पे पैसे ला राख्या सै।",
	},

	["Do you think my cat knows about the feminist movement?"] = {
		["_default"] = "Do you think my cat knows about the feminist movement?",
		["Khariboli"] = "क्या तुम्हें लगता है कि मेरी बिल्ली को नारीवादी आंदोलन के बारे में पता है?",
		["Punjabi"] = "ਕੀ ਤੁਹਾਨੂੰ ਲੱਗਦਾ ਹੈ ਕਿ ਮੇਰੀ ਬਿੱਲੀ ਨੂੰ ਨਾਰੀਵਾਦੀ ਹਲਚਲ ਬਾਰੇ ਪਤਾ ਹੈ?",
		["Bhojpuri"] = "का लागता कि हमार बिल्ली के नारीवाद आंदोलन के बारे में पता बा?",
		["Haryanvi"] = "तने लागे सै मेरी बिल्ली फेमिनिस्ट मूवमेंट जाणे सै?",
	},

	["Norbert has a face you’d want to punch. Not because there is anything wrong with the face itself, but just because it's his and he is mean."] = {
		["_default"] = "Norbert has a face you’d want to punch. Not because there is anything wrong with the face itself, but just because it's his and he is mean.",
		["Khariboli"] = "नॉर्बर्ट का चेहरा ऐसा है जिसे आप घूंसा मारना चाहेंगे। चेहरा खुद में गलत नहीं है, बस इसलिए क्योंकि यह उसका है और वह दुष्ट है।",
		["Punjabi"] = "ਨੋਰਬਰਟ ਦਾ ਚਿਹਰਾ ਐਧਾਂ ਹੈ ਜਿਸ ਨੂੰ ਤੁਸੀਂ ਥੱਪੜ ਮਾਰਨਾ ਚਾਹੋਗੇ। ਚਿਹਰੇ ਵਿੱਚ ਖੁਦ ਕੋਈ ਗਲਤ ਨਹੀਂ, ਸਿਰਫ ਇਸ ਲਈ ਕਿਉਂਕਿ ਇਹ ਉਸਦਾ ਹੈ ਅਤੇ ਉਹ ਮਾੜਾ ਹੈ।",
		["Bhojpuri"] = "नॉर्बर्ट के मुंह ऐसन बा कि आप घूंसा मारना चाही। खुद मुंह में कवनो गलती नइखे, बस एह खातिर कि ऊ ओकरा के बा आ ऊ खराब बा।",
		["Haryanvi"] = "नॉर्बर्ट का मुंह ऐसा सै कि तू थप्पड़ मारणा चाहे। मुंह में कुछ गलत ना सै, बस क्योंकि वो उसका सै और वो बदमाश है।",
	},

	["Refusing to have an opinion, is a way of having one, isn’t it?"] = {
		["_default"] = "Refusing to have an opinion, is a way of having one, isn’t it?",
		["Khariboli"] = "कोई राय रखने से इनकार करना भी एक राय रखने का तरीका है, है न?",
		["Punjabi"] = "ਇੱਕ ਰਾਏ ਨਾ ਰੱਖਣਾ ਵੀ ਇੱਕ ਰਾਏ ਰੱਖਣ ਦਾ ਤਰੀਕਾ ਹੈ, ਹੈ ਨਾ?",
		["Bhojpuri"] = "राय ना रखल भी एगो राय राखे के तरीका ह, ना?",
		["Haryanvi"] = "राय ना रखना भी एक तरह की राय सै, है ना?",
	},

	["My name is Len, short for Lenjamin."] = {
		["_default"] = "My name is Len, short for Lenjamin.",
		["Khariboli"] = "मेरा नाम लेन है, जो लेनजमिन का छोटा रूप है।",
		["Punjabi"] = "ਮੇਰਾ ਨਾਮ ਲੈਨ ਹੈ, ਜੋ ਲੇਨਜਮਿਨ ਦਾ ਸੰਖੇਪ ਹੈ।",
		["Bhojpuri"] = "हमार नाम लेन बा, लेनजमिन के छोट रूप।",
		["Haryanvi"] = "म्हारा नाम लेन सै, लेनजमिन का छोटा रूप।",
	},

	["Do you think if we get enough of us together, we could overthrow the gigantic person that runs our city?"] = {
		["_default"] = "Do you think if we get enough of us together, we could overthrow the gigantic person that runs our city?",
		["Khariboli"] = "क्या तुम्हें लगता है कि अगर हम पर्याप्त लोग एक साथ हो जाएँ, तो हम हमारे शहर को चलाने वाले विशाल व्यक्ति को हटा सकते हैं?",
		["Punjabi"] = "ਕੀ ਤੁਹਾਨੂੰ ਲੱਗਦਾ ਹੈ ਕਿ ਜੇ ਅਸੀਂ ਕਾਫ਼ੀ ਲੋਕ ਇਕੱਠੇ ਹੋ ਜਾਈਏ ਤਾਂ ਅਸੀਂ ਆਪਣੇ ਸ਼ਹਿਰ ਨੂੰ ਚਲਾਉਣ ਵਾਲੇ ਦੈਤਨੁਮਾ ਵਿਅਕਤੀ ਨੂੰ ਹਟਾ ਸਕਦੇ ਹਾਂ?",
		["Bhojpuri"] = "का तोहके लागल कि अगर हमनी के बहुत लोग मिल जाईं त हम शहर चला रहल बहुते बड़े आदमी के हटा सकेनी?",
		["Haryanvi"] = "तने लागे सै कि अगर हम सारे जमा हो जा, तो हम अपने शहर चलाण वाले उस बड़े आदमी ने हटा सकेंगे?",
	},

	["I cannot become what I need to be, by remaining what I am."] = {
		["_default"] = "I cannot become what I need to be, by remaining what I am.",
		["Khariboli"] = "मैं वही बने रहकर वह नहीं बन सकता जो मुझे बनने की आवश्यकता है।",
		["Punjabi"] = "ਮੈਂ ਉਹ ਨਹੀਂ ਬਣ ਸਕਦਾ ਜੋ ਮੈਨੂੰ ਬਣਨ ਦੀ ਲੋੜ ਹੈ, ਜੇ ਮੈਂ ਉਹੀ ਰਹਾਂ ਜੋ ਮੈਂ ਹਾਂ।",
		["Bhojpuri"] = "हम जे बा उहे रहके हम जे बने के जरूरत बा उ ना बन सकी।",
		["Haryanvi"] = "मैं जो हूं, वहीं रहकर मैं जो बनना चाहिए, वो नहीं बन सकता।",
	},

	["It’s ok that you’re not who you thought you’d be."] = {
		["_default"] = "It’s ok that you’re not who you thought you’d be.",
		["Khariboli"] = "यह ठीक है कि आप वह नहीं हैं जो आपने सोचा था कि आप होंगे।",
		["Punjabi"] = "ਇਹ ਠੀਕ ਹੈ ਕਿ ਤੁਸੀਂ ਉਹ ਨਹੀਂ ਹੋ ਜੋ ਤੁਸੀਂ ਸੋਚਦੇ ਸੀ ਤੁਸੀਂ ਹੋਵੋਗੇ।",
		["Bhojpuri"] = "ई ठीक बा कि आप वैसना नइखी जवन सोचे रहीं।",
		["Haryanvi"] = "ठीक सै कि तू वो ना है जो तू सोचता था तू होगा।",
	},

	["If you feel like everyone else hates you, you need sleep. If you feel like you hate everyone else, you need to eat."] = {
		["_default"] = "If you feel like everyone else hates you, you need sleep. If you feel like you hate everyone else, you need to eat.",
		["Khariboli"] = "अगर आपको लगता है कि सभी आपसे नफरत करते हैं, तो आपको नींद की जरूरत है। अगर आपको लगता है कि आप सभी से नफरत करते हैं, तो आपको खाना चाहिए।",
		["Punjabi"] = "ਜੇ ਤੁਹਾਨੂੰ ਲੱਗਦਾ ਹੈ ਕਿ ਹੋਰ ਸਾਰੇ ਤੁਹਾਨੂੰ ਨਫਰਤ ਕਰਦੇ ਹਨ, ਤਾਂ ਤੁਹਾਨੂੰ ਨੀਂਦ ਦੀ ਲੋੜ ਹੈ। ਜੇ ਤੁਹਾਨੂੰ ਲੱਗਦਾ ਹੈ ਕਿ ਤੁਸੀਂ ਹੋਰ ਸਾਰਿਆਂ ਨੂੰ ਨਫਰਤ ਕਰਦੇ ਹੋ, ਤਾਂ ਤੁਹਾਨੂੰ ਖਾਣ ਦੀ ਲੋੜ ਹੈ।",
		["Bhojpuri"] = "अगर लगे कि सब लोग आपसे नफरत कर रहल बा, त नींद जरूरी बा। अगर लगे कि आप सब से नफरत करत बानी, त खाना जरूरी बा।",
		["Haryanvi"] = "अगर तने लगै कि सब तुझसे नफरत करते हैं, तो तने नींद चाहिए। अगर तने लगै कि तू सबसे नफरत करे है, तो तने खाना चाहिए।",
	},

	["Do you think whoever runs this city knows what a penguin looks like? I don't, and I am really curious."] = {
		["_default"] = "Do you think whoever runs this city knows what a penguin looks like? I don't, and I am really curious.",
		["Khariboli"] = "क्या तुम्हें लगता है कि जो भी इस शहर को चलाता है, उसे पता है कि पेंगुइन कैसा दिखता है? मुझे नहीं पता, और मैं बहुत जिज्ञासु हूँ।",
		["Punjabi"] = "ਕੀ ਤੁਸੀਂ ਸੋਚਦੇ ਹੋ ਕਿ ਜੋ ਵੀ ਇਸ ਸ਼ਹਿਰ ਨੂੰ ਚਲਾਉਂਦਾ ਹੈ ਉਹਨਾਂ ਨੂੰ ਪੇਂਗੁਇਨ ਕਿਵੇਂ ਲੱਗਦੇ ਹਨ? ਮੈਂ ਨਹੀਂ ਜਾਣਦਾ ਅਤੇ ਮੈਂ ਬਹੁਤ ਉਤਸ਼ੁਕ ਹਾਂ।",
		["Bhojpuri"] = "का लागेला कि जवन आदमी शहर चलावत बा उ जानेला कि पेंगुइन कइसे देखाला? हम न जानत हईं आ हम बहुत जिज्ञासु बानी।",
		["Haryanvi"] = "तने लागै कि जो शहर चलावे है, वो जाने सै पेंगुइन कैसा दिखे? मैं नहीं जानूं और मैं बड़ा जिज्ञासु हूं।",
	},

	["I think ultimately you become whoever would have saved you that time that no one did."] = {
		["_default"] = "I think ultimately you become whoever would have saved you that time that no one did.",
		["Khariboli"] = "मुझे लगता है कि अंततः आप वही बन जाते हैं जो उस समय आपको बचा सकता था जब किसी ने नहीं बचाया।",
		["Punjabi"] = "ਮੈਨੂੰ ਲੱਗਦਾ ਹੈ ਕਿ ਆਖ਼ਰਕਾਰ ਤੁਸੀਂ ਉਹੀ ਬਣ ਜਾਓਗੇ ਜੋ ਉਸ ਵੇਲੇ ਤੁਹਾਨੂੰ ਬਚਾ ਸਕਦਾ ਸੀ ਜਦ ਕੋਈ ਨਹੀਂ ਬਚਾ।",
		["Bhojpuri"] = "लगता बा कि आखिर में आदमी ओह बनेला जवन उ वक़्त उनकरा बचा सकत रहे, जब केहू ना बचावल।",
		["Haryanvi"] = "मने लागै है कि आखिर में तू वही बन जावे है जो उस वक्त तने बचा सकता था जब किसी ने ना बचाया।",
	},

	["I learn something everyday. And a lot of times, it’s that what I learned yesterday, was wrong."] = {
		["_default"] = "I learn something everyday. And a lot of times, it’s that what I learned yesterday, was wrong.",
		["Khariboli"] = "मैं हर दिन कुछ सीखता हूं। और कई बार, यह कि जो मैंने कल सीखा था, वह गलत था।",
		["Punjabi"] = "ਮੈਂ ਹਰ ਰੋਜ਼ ਕੁਝ ਸਿੱਖਦਾ ਹਾਂ। ਅਤੇ ਕਈ ਵਾਰੀ, ਇਹ ਕਿ ਜੋ ਮੈਂ ਕੱਲ੍ਹ ਸਿੱਖਿਆ ਸੀ, ਉਹ ਗਲਤ ਸੀ।",
		["Bhojpuri"] = "हम हर दिन कुछ ना कुछ सीखेलें। आ बहुते बेर, एही सीखेला कि काल्ह जे सीखनी, ऊ गलत रहे।",
		["Haryanvi"] = "मैं रोज कुछ सीखूं हूं। और कई बार पता चलता है कि कल जो सीखा था, वो गलत था।",
	},

	["I know that when the world falls apart, raccoons will never judge me. They will only haunt my waking nightmares with their tiny, tiny hands."] = {
		["_default"] = "I know that when the world falls apart, raccoons will never judge me. They will only haunt my waking nightmares with their tiny, tiny hands.",
		["Khariboli"] = "मैं जानता हूं कि जब दुनिया बिखर जाएगी, रैकून कभी भी मुझे नहीं आंकेंगे। वे बस अपने छोटे-छोटे हाथों से मेरी जागती दुःस्वप्नों को परेशान करेंगे।",
		["Punjabi"] = "ਮੈਂ ਜਾਣਦਾ ਹਾਂ ਕਿ ਜਦੋਂ ਦੁਨੀਆਂ ਡਿੱਗ ਜਾਂਦੀ ਹੈ, ਰੈਕੂਨ ਕਦੇ ਵੀ ਮੇਰਾ ਮੁਲਾਂਕਣ ਨਹੀਂ ਕਰਨਗੇ। ਉਹ ਸਿਰਫ਼ ਆਪਣੇ ਛੋਟੇ ਛੋਟੇ ਹੱਥਾਂ ਨਾਲ ਮੇਰੇ ਸੁਪਨੇ ਡਰਾਉਣੇ ਬਣਾਉਣਗੇ।",
		["Bhojpuri"] = "हम जानतानी कि जब दुनिया टूट जाई, रैकून हमनी के जज ना करी। ऊ बस अपना छोट-छोटा हाथ से हमार जागल डरावन सपना के सताइहन।",
		["Haryanvi"] = "मैं जानूं हूँ कि जब दुनिया टूट जाएगी, रैकून मेरी निंदा नहीं करेंगे। वो सिर्फ अपने छोटे-छोटे हाथों से मेरे जागते दुःस्वप्नों में डरावना काम करेंगे।",
	},

	["I've got to be careful going in search of adventure. It’s ridiculously easy to find."] = {
		["_default"] = "I've got to be careful going in search of adventure. It’s ridiculously easy to find.",
		["Khariboli"] = "मुझे साहसिकता की खोज में सावधान रहना होगा। इसे ढूंढ़ना हास्यास्पद रूप से आसान है।",
		["Punjabi"] = "ਮੈਨੂੰ ਮੁਹਿੰਮ ਦੀ ਖੋਜ ਵਿੱਚ ਸਾਵਧਾਨ ਰਹਿਣਾ ਚਾਹੀਦਾ ਹੈ। ਇਸਨੂੰ ਲੱਭਣਾ ਬਹੁਤ ਆਸਾਨ ਹੈ।",
		["Bhojpuri"] = "हमके रोमांच खोजत समय सतर्क रहे के पड़ी। ई ढूँढल बहुत आसान बा।",
		["Haryanvi"] = "मने रोमांच की तलाश में सावधान रहणा चाहिए। इसे ढूंढना मजाकिया तौर पर आसान सै।",
	},

	["Latest news: A new type of deodorant has been invented! It does exactly the same thing as the old ones."] = {
		["_default"] = "Latest news: A new type of deodorant has been invented! It does exactly the same thing as the old ones.",
		["Khariboli"] = "ताजा खबर: एक नया प्रकार का डियोड्रेंट आविष्कार किया गया है! यह बिल्कुल वही काम करता है जो पुराने वाले करते थे।",
		["Punjabi"] = "ਤਾਜ਼ਾ ਖ਼ਬਰ: ਡੀਓਡੋਰੈਂਟ ਦੀ ਇੱਕ ਨਵੀਂ ਕਿਸਮ ਦਾ ਆਵਿਸ਼ਕਾਰ ਹੋਇਆ ਹੈ! ਇਹ ਬਿਲਕੁਲ ਉਹੀ ਕਰਦਾ ਹੈ ਜੋ ਪੁਰਾਣੇ ਕਰਦੇ ਸਨ।",
		["Bhojpuri"] = "ताज़ा खबर: नया तरह के डिओडरेंट बनावल गइल बा! ई बिलकुल ओही काम करेला जवन पुराना वाला करत रहे।",
		["Haryanvi"] = "ताजा खबर: एक नए तरह का डियोड्रेंट बनाया गया सै! ये वही काम करता है जो पुराने करते थे।",
	},

	["If god isn't real then why does the palm of a man fit so perfectly against the throat of a goose?"] = {
		["_default"] = "If god isn't real then why does the palm of a man fit so perfectly against the throat of a goose?",
		["Khariboli"] = "यदि भगवान असली नहीं हैं तो फिर आदमी की हथेली हंस की गर्दन पर इतनी अच्छी तरह क्यों फिट बैठती है?",
		["Punjabi"] = "ਜੇ ਰੱਬ ਹਕੀਕਤ ਨਹੀਂ ਹੈ ਤਾਂ ਫਿਰ ਇੱਕ ਆਦਮੀ ਦੀ ਹਥੇਲੀ ਹੰਸ ਦੀ ਗਰਦਨ 'ਤੇ ਇਤਨੀ ਅੱਛੀ ਤਰ੍ਹਾਂ ਕਿਉਂ ਫਿੱਟ ਹੁੰਦੀ ਹੈ?",
		["Bhojpuri"] = "अगर भगवान असली ना हवन त आदमी के हथेली हंस के गरदन पर इतना बढ़िया काहे फिट होले?",
		["Haryanvi"] = "अगर भगवान असली ना हो तो आदमी की हथेली हंस की गर्दन पे इतनी सही फिट क्यों बैठती है?",
	},

	["You Need to Build A Water Source"] = {
		["_default"] = "You Need to Build A Water Source",
		["Khariboli"] = "आपको एक जल स्रोत बनाना चाहिए",
	},

	["You Need to Build Water Pipes"] = {
		["_default"] = "You Need to Build Water Pipes",
		["Khariboli"] = "आपको पानी के पाइप बनाना चाहिए",
	},

	["You Need to Build A Power Source"] = {
		["_default"] = "You Need to Build A Power Source",
		["Khariboli"] = "आपको एक बिजली स्रोत बनाना चाहिए",
	},

	["You Need to Build Power Lines"] = {
		["_default"] = "You Need to Build Power Lines",
		["Khariboli"] = "आपको बिजली की लाइनें बनानी चाहिए",
	},

	["You Need to Build Roads"] = {
		["_default"] = "You Need to Build Roads",
		["Khariboli"] = "आपको सड़कें बनानी चाहिए",
	},

	["Roads must connect back to the highway or citizens cant travel to your city!"] = {
		["_default"] = "You Need to Build Roads",
		["Khariboli"] = "You Need to Build Roads",
	},
}
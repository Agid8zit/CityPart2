...
local SUPPORT_WATER_SKIP_MAX = 3
local SHALLOW_WATER_DEPTH   = 2.5

local function isCliffInstance(inst)
	while inst do
		local name = (inst.Name or ""):lower()
		if name:find("cliff") then
			return true
		end
		inst = inst.Parent
	end
...
local function sampleSupportAtOffset(part : BasePart, offset : Vector3)
	local basePoint = part.CFrame:PointToWorldSpace(offset)
	local origin = basePoint - Vector3.new(0, part.Size.Y * 0.5 + 0.05, 0)
	local ignore = { part }
	local lastWaterDepth = nil

	for _ = 1, SUPPORT_WATER_SKIP_MAX do
		local params = RaycastParams.new()
		params.FilterDescendantsInstances = ignore
		params.FilterType  = Enum.RaycastFilterType.Exclude
		params.IgnoreWater = false

		local result = workspace:Raycast(origin, Vector3.new(0, -SUPPORT_RAY_LENGTH, 0), params)
		if not result then
			return "Void", nil
		end
		...
		local depth = basePoint.Y - result.Position.Y

		if isCliffInstance(inst) then
			return "Cliff", depth
		end

		local material = result.Material
		local hitIsWater =
			(inst:IsA("Terrain") and material == Enum.Material.Water)
			or (not inst:IsA("Terrain") and inst.Material == Enum.Material.Water)

		if hitIsWater then
			if depth <= SHALLOW_WATER_DEPTH then
				return "Land", depth
			end
	...
	return "Water", lastWaterDepth
end

local function supportIsGap(kind)
	return kind == "Water" or kind == "Void"
end

local function supportIsAnchor(kind)
	return kind == "Cliff"
end

local function getSupportTypeForCell(part : BasePart)
	local counts = { Cliff = 0, Water = 0, Land = 0, Void = 0 }
	local landDepth = math.huge
	local gapDepth  = 0

	for _, offset in ipairs(SUPPORT_SAMPLE_OFFSETS) do
		local kind, depth = sampleSupportAtOffset(part, offset)
		counts[kind] = (counts[kind] or 0) + 1
		if kind == "Land" and depth then
			landDepth = math.min(landDepth, depth)
		elseif supportIsGap(kind) and depth then
			gapDepth = math.max(gapDepth, depth)
		end
	end

	if counts.Cliff > 0 then
		local d = landDepth ~= math.huge and landDepth or nil
		return "Cliff", d
	end
	if counts.Land > 0 and counts.Land >= counts.Water then
		local d = landDepth ~= math.huge and landDepth or nil
		return "Land", d
	end
	if counts.Water > 0 then
		local d = (gapDepth > 0) and gapDepth or nil
		return "Water", d
	end
	return "Void", nil
end
	...
	task.spawn(function()
		...
		if predefinedRoads then
			...
			for i, cell in ipairs(gridList) do
				...
				if bridgeAsset then
					if supportIsGap(support) then
						bridgingActive = true
						asset = bridgeAsset
					elseif bridgingActive and supportIsAnchor(support) then
						asset = bridgeAsset
